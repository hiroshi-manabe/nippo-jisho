...
| marked `o` (`ô`, `ǒ`, and related forms) | only 28/48 and 26/48 exact overall; an initially promising 43/46 single-page N-gram result fell to 1,522/2,152 (70.72%) in page-level ten-fold validation | Do not let OCR infer the presence of a mark. Once a mark is visually established, treat the N-gram score only as weak linguistic evidence and enlarge the source. |
...
exists. Although the selected 2-gram chose `ô` versus `ǒ` in 43/46 held-out
f151 occurrences, including all 20 members of the `Vôqina`/`Vôyubi` family,
page-level ten-fold validation over `f13`–`f151` reached only 70.72% overall and
