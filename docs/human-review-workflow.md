...
Standalone editorial abbreviations inside Roman-type Japanese material, including `i` and Latin `l` (*vel*, “or”), are excluded without suppressing the surrounding Japanese reading. When a Roman-type word is physically divided across two lines, the opening remainder on the second line is not displayed as an independent kana reading; this is detected from the preceding Roman run's terminal hyphen rather than from indentation alone.
