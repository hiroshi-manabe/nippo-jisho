...
The durable note belongs to the Level 1 line annotation and may record linguistic reasoning, uncertainty, damage, or another lasting observation. The Message to AI belongs only to the review workspace and correction Issue. Any nonempty message requires AI inspection. The independent “Request a second opinion” checkbox also requests inspection, including with no message and no text or note change. A checked box is serialized as `second_opinion: true`; leaving it unchecked never exempts a nonempty message from review. The processor mechanically applies settled rows, pauses on rows with messages or checked second-opinion requests, and never promotes a temporary message into the durable note without an explicit note change.
...
For `nu` followed by a separate `ua`/`uo`, preserve the first syllable: `Inuuo` → イヌヲ, unlike `-nuo` above. Both `cc` (`mocco` → モッコ) and mixed `cq` represent doubled k (`fiacqi` → ヒャッキ; `ichinifiacqizzutçu` → イチニヒャッキヅツ). Palatalized `eǒ`/`eô` also receive ョゥ after the applicable consonants (`Reǒginno` → リョゥヂンノ, `Reôri` → リョゥリ); unmarked `eo` is not globally rewritten. The simplified hint still merges open and closed long o. Takemura explicitly associates `Riŏ～Reŏ` with りやう and `Reô～Riô` with れう, while discussing possible finer phonetic differences between the i/e variants: [research report](https://matsushita-konosuke-zaidan.or.jp/data/promotion_research/pdf/10-040.pdf). None of these derived rules normalizes the diplomatic spelling.
...
2. After baseline validation finishes, open a blank Issue tab and attempt to copy the complete payload. On success, navigate the tab to the Issue composer. Do not open a tab before the network check in either single-page or multi-page mode.
3. If copying fails, close the blank tab and show the complete JSON in a read-only selectable textarea, with **Copy JSON** and **Open Issue** buttons. The extra copy button supplies a fresh user gesture; manual copying remains available. If the tab was blocked, also show this dialog rather than navigating away and hiding the payload. Never claim copying succeeded when it failed.
...
`before` and `note_before` protect against silently applying a correction to text or durable annotation that has subsequently changed. In schema 3, an absent or empty `message` with no `second_opinion: true` means that the human reviewer has settled the exact correction and asks for mechanical application. A nonempty message or `second_opinion: true` requires AI inspection. The processor retains schema-2 compatibility for already-created Issues, treating an old `comment` as a temporary message and `second_opinion: true` as requiring inspection, but the interface creates schema 3 only.

Issue processing is deliberately two-stage. First validate the page, base version, line IDs, text, notes, and correction notation, then mechanically apply every schema-3 change without a message or second-opinion request. Do not reopen those glyph judgments merely because machine vision would have read them differently. Next, perform the detailed linguistic-first and visual review below only for rows carrying a message or `second_opinion: true`. Do not commit, push, or close the Issue until that set is settled. If neither messages nor second-opinion requests are present, the validated mechanical application may proceed directly through tests, commit, deployment, and Issue closure.
...
Omit `ISSUE_NUMBER` to select exactly one oldest open Issue by creation time; an explicit number selects that Issue instead. If there are no open Issues, the command reports that and exits successfully. Unsupported content, errors, or required review stop processing; it never skips to another Issue or processes a queue. `finalize` still requires an explicit Issue number.

It fetches one schema-2 or schema-3 payload, verifies that the Issue is open and its base commit exists, validates every stable line ID and baseline value, resolves lightweight `*word`, `[roman]`, and `{italic}` notation, and applies all unmessaged changes to the page's editable source. Canonical pages update compact Level 1 Markdown; machine-provisional pages update their candidate package in place and remain provisional. Unknown lines, changed baseline text or notes, ambiguous notation, multiple payload blocks, and schema 1 are hard errors. Existing layout metadata is retained through textual edits.

After application, the processor writes its report before validation, recompiles canonical interchange JSON and views when applicable, rebuilds the public review artifact, and runs the complete test suite. A validation failure leaves the Issue open and permits a repeat invocation to recognize already-applied targets. If neither an AI message nor a second-opinion request is present, it updates correction history, commits, pushes `main`, verifies deployment, comments on the Issue, and closes it. A failure before deployed verification leaves the Issue open.

If one or more schema-3 changes contain a nonempty `message` or `second_opinion: true`, the processor applies and validates only the unflagged subset, writes `build/correction-issues/issue-N.json`, prints its path, and exits with status 3. No history update, commit, push, deployment, or Issue closure occurs. Each flagged item begins with `"decision": "pending"`. Schema-2 `second_opinion` items follow the same compatibility path. After review, the machine reviewer records one of:
...
abbreviation letters such as `i.`, `l`, and `S.` retain the typeface toggle;
plain lowercase `a` remains excluded as before.
...
The line editor provides a compact transcription-character palette. Number keys `0`–`9` insert the character shown on the corresponding button while the transcription field is focused: `ß`, `ſ`, `ç`, tilde, grave, acute, `ǒ`, `ǔ`, `ô`, and `û`. The `0` (`ß`) button sits immediately to the left of `1` (`ſ`). The three accent buttons apply their mark to a selected single letter or to the letter immediately before the caret; output is normalized to NFC except for combinations such as `q̃` that have no precomposed Unicode character. A **Literal digits** control temporarily restores ordinary numeric input for the uncommon correction that must introduce or change a printed number.
...
An asterisk immediately before or after a word supplies a lightweight proposal annotation without a dedicated UI control. When the word has one tilde in a maximal adjacent-vowel sequence of exactly two vowels, `*word` or `word*` means **move that tilde to the other vowel and change nothing else**: `*mãos` becomes `maõs`, and `dalguã*` becomes `dalgũa`. On saving, the browser resolves the instruction, removes the marker and normalizes the affected word to NFC. The displayed edit and submitted JSON contain the resulting spelling. Existing saved drafts are also resolved when loaded or reconciled in selection mode, allowing an incorporated correction to clear by ordinary text comparison. Notes, AI messages and second-opinion requests are not converted or discarded. A resolved spelling contains no marker and is not swapped again on later saves. Ambiguous markers are preserved with an **Unresolved tilde marker** warning; supply the intended spelling explicitly. The server retains marker support for older submissions and rejects unresolved or ambiguous instructions. An asterisk does not mean general uncertainty and is never written into canonical Level 1 text.
