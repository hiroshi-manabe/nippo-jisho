...
Issue #46 completed `f23`–`f32` with 97 short-`st` replacements and 14 confirmed long forms. Its 28 visually proposed initial checks did not help the human review and were all cleared. That negative result is retained in `pilot/st-ligature-prediction-f23-f32.md` and `pilot/st-ligature-prechecks-f23-f32.tsv` as provenance, but prechecks no longer influence the live task. The current `f33`–`f100` interface starts every candidate unchecked and leaves the entire classification to the human reviewer.
