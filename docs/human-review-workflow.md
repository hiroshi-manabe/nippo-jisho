...
The application retains the exact IIIF JPEG URL for every successfully visited leaf in an in-memory cache of `Image` objects for the lifetime of the browser tab. It makes no speculative or neighboring-page requests: a leaf begins loading only when the reader opens it, and revisiting that leaf reuses its retained image. This conservative policy follows a concurrent five-image trial that produced HTTP `429 Too Many Requests` responses from Gallica and a later sequential-preload trial that was still insufficiently reliable in ordinary use. Failed entries are removed rather than retained, and a current-page failure receives bounded backoff retries plus an explicit retry control. Visible scan elements receive their `src` only after the retained image has loaded, so a temporary upstream failure produces a loading or retry state rather than a broken-image icon. The cache is intentionally unbounded within one tab for now; if a reader visits hundreds of leaves, memory use may grow, although the browser remains free to discard decoded pixels under memory pressure. Closing or reloading the tab releases the cache.
