...
The dedicated line-end-hyphen interface now covers every processed leaf from `f44` through `f100` in one task. It supersedes the former `f44`–`f53` trial UI at the same route; its distinct `f44-f100` scope key intentionally prevents the obsolete browser selection state from being restored. It lists every column line whose current transcription ends in `-`, and its submission contains only checked removals. In-progress selections are stored by task scope rather than deployment commit, so geometry-only deployments and ordinary refreshes preserve them. Each stored selection also carries its source-line version; if that transcription later changes, only the affected stale selection is discarded. The interface migrates the original commit-scoped selection array on first load. Page images are released after their right-edge crops have been drawn so scrolling through the larger 57-page set does not retain every decoded master image.

The dedicated tilde-position interface covers the still-unadjudicated entries in `adjacent-vowel-tilde-audit.tsv` from `f39` through `f100`. A candidate is included only when the marked vowel belongs to a maximal run of exactly two adjacent vowels and exactly one member of that pair carries a tilde. Human-confirmed records are excluded. Each card shows the current token and the sole alternative obtained by moving the tilde to the other vowel; checking the card requests that swap and leaving it unchecked requests no change. Repeated tokens on one line retain occurrence numbers so submissions are unambiguous. The up and down arrow keys move focus between cards and Space checks or unchecks the focused card. Selections are stored by task scope and source-line version, then copied as a versioned `tilde-carrier-audit` JSON payload for a dedicated GitHub Issue.

The `ſt` / `st` specialist task includes every literal `ſt` occurrence in column text, including repeated occurrences on one line. Its first two scopes used tightly estimated occurrence-level crops, but the larger f33–f100 scope exposed a systematic weakness: variable indentation, word spacing, and mixed type widths could place the actual sort outside the card even when the underlying line geometry was correct. The live task therefore shows the complete reviewed printed-line rectangle and highlights the exact transcribed occurrence in the full transcription below it. This is less dense, but it removes horizontal position estimation from the critical reading path; repeated occurrences still receive independent cards and stable occurrence numbers. The audit starts from the empirically supported short-`s` default: an unchecked card requests provisional `ſt` → `st`, while a checked card retains a positively identified long `ſ` + `t`. This inverted selection meaning is stated in the header and repeated in the live retain/convert counts. Left and right arrows advance in reading order, up and down choose the closest card in the adjacent visual row, and Space toggles the focused card. Retained selections persist by task scope and source-line version. Schema 2 submissions are deliberately positive-only and compact: the base commit pins the complete candidate inventory, `retain` lists checked long-`ſ` keys, and `unchecked: provisional-st` classifies every unlisted candidate as provisionally short without serializing hundreds of individual replacements. This preserves the distinction between positive human findings and an incomplete default classification, allowing deferred candidates—particularly f51 onward in the present pass—to be reconstructed for a later audit. Issue #45 completed f13–f22 with 108 short-`st` replacements and 28 confirmed long forms. Those retained decisions are versioned in `pilot/st-ligature-audit.tsv`; a matching current occurrence is excluded from the unresolved UI, while any later source-line change makes its confirmation stale and returns it for review.

Issue #46 completed `f23`–`f32` with 97 short-`st` replacements and 14 confirmed long forms. Its 28 visually proposed initial checks did not help the human review and were all cleared. That negative result is retained in `pilot/st-ligature-prediction-f23-f32.md` and `pilot/st-ligature-prechecks-f23-f32.tsv` as provenance, but prechecks no longer influence the live task.

Issue #47 applied the deliberately incomplete `f33`–`f100` pass: 154 checked occurrences are confirmed long `ſ` + `t`, while 631 unchecked occurrences are represented as provisional short `st`. The pass was stopped because the crops became too costly to read reliably, especially from `f51` onward. The full pinned inventory—including original text, offsets, line versions, crops, and both kinds of decision—is preserved in `pilot/st-ligature-classification-f33-f100.json`. This snapshot, rather than the rewritten transcription alone, is the authority for reconstructing a better-layout follow-up audit. Confirmed long forms are also copied into the ordinary versioned ledger; the provisional short forms remain explicitly revisable and must not be cited as exhaustively human-confirmed allographs.

Tilde-audit word crops must not scale a short printed line as though it filled the entire column. Horizontal placement uses a fixed full-line character measure plus the transcription's indentation, so short continuation lines remain at their printed edge. A candidate at the end of a long line is anchored to the outer rule even when terminal punctuation or whitespace follows the token; that suffix must not cause the last letter and its tilde to be clipped. These crops also retain more space above the reviewed line than ordinary reading crops: a rectangle sufficient to recognize the letter bodies may still clip the very tilde this task is intended to adjudicate. From `f60` onward, the tilde-audit crop adds approximately one printed line of vertical context on both sides. This compensates in the specialist UI for reviewed geometry that is readable but vertically displaced, while leaving the canonical line rectangles used by the main interface unchanged.
...
If a correction intentionally invalidates a content-specific regression assertion, the machine reviewer updates that assertion and records its repository-relative path in the report's `additional_paths` list. Finalization will then permit and stage that explicit path; it never broadens the staged set merely because another tracked file happens to be dirty.
...
The correction field also has one intentionally asymmetric typeface annotation. Select Japanese text that has been incorrectly absorbed into an italic Portuguese explanation and press **Roman**; the editor wraps the selection as `[Fotoqe]`. Square brackets are reserved for this purpose in correction proposals and are not literal transcription characters. The row preview removes the brackets, renders the selected text in roman type, and highlights the styling correction. The submitted JSON retains `[Fotoqe]` so the Issue remains readable; Issue application converts that span into the corresponding canonical Markdown run boundary. There is no inverse italic annotation because the observed correction need is embedded Japanese in explanatory Portuguese, not Portuguese embedded in Japanese source text. If a genuine square bracket is ever encountered, the notation will be revised interactively rather than carrying an unused escaping system now.
