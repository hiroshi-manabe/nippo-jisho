...
The compact `ſt` / `st` specialist trial covers the first ten column-form leaves, `f13` through `f22`. It includes every literal `ſt` occurrence in column text, including repeated occurrences on one line, and displays a tight occurrence-level crop in a horizontally packed tile grid. Tight horizontal crops must not assume that every printed line contains a fixed number of characters: the occurrence estimate combines the actual transcription length with a type-width cap derived from the recorded line height, so a shorter line that nevertheless fills the measure does not put a late word outside its tile. An occurrence that still falls outside this estimate receives an explicit crop override; `f18/c2a-l005#1` is the first recorded case, human-confirmed from the full scan as `Eſtrella`. The audit starts from the empirically supported short-`s` default: an unchecked tile requests `ſt` → `st`, while a checked tile retains a positively identified long `ſ` + `t`. This inverted selection meaning is stated in the header and repeated in the live retain/convert counts. Left and right arrows advance in reading order, up and down choose the closest tile in the adjacent visual row, and Space toggles the focused tile. Retained selections persist by task scope and source-line version; copied JSON contains only the unchecked replacements, with stable occurrence numbers so multiple candidates on one line remain independently addressable. Issue #45 completed the first scope with 108 short-`st` replacements and 28 confirmed long forms. Those retained decisions are versioned in `pilot/st-ligature-audit.tsv`; a matching current occurrence is excluded from the unresolved UI, while any later source-line change makes its confirmation stale and returns it for review.

For `f23`–`f32`, the completed first scope is also a labeled control set. Initial checks may be supplied only by a recorded method whose result has been tested against held-out human labels; failed model output is not silently promoted into the UI. The first crop-level classifier failed this test and was rejected. The present 28 initial checks instead come from direct comparison with labeled control specimens, are bound to exact source-line versions, and are visibly described as predictions that the human may change or clear. A source-line mismatch suppresses the stale precheck. The experiment and precheck provenance are recorded in `pilot/st-ligature-prediction-f23-f32.md` and `pilot/st-ligature-prechecks-f23-f32.tsv`.
