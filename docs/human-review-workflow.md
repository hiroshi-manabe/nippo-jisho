...
A trailing asterisk supplies a second lightweight proposal annotation without adding a dedicated UI control. When a word has one tilde in an adjacent-vowel sequence and exactly one unambiguous alternative carrier, appending `*` means **move that tilde to the other vowel and change nothing else**: submitted `mãos*` requests `maõs`, and `dalguã*` requests `dalgũa`. The asterisk is an instruction only; Issue application removes it, moves the combining mark, normalizes the result to NFC where possible, and never writes `*` into canonical Level 1 text. It does not mean general uncertainty. If the local sequence has three or more plausible carriers, the word has multiple tildes, or any other reading is in question, the reviewer must instead supply the complete intended spelling or explain it in a comment. No special editor behavior is required: the reviewer types the suffix directly.
