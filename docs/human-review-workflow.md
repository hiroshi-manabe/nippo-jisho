...
The application retains the IIIF JPEGs for every successfully visited leaf in in-memory `Image` caches for the lifetime of the browser tab. It makes no speculative or neighboring-page requests. On first visit it immediately requests a 1000-pixel-wide preview and displays it as soon as it arrives. After the preview succeeds and the leaf remains current for a short dwell period, a dedicated manager may request the 2200-pixel HD image and replace the preview when it arrives. Revisiting a leaf reuses either or both retained versions.

The HD manager owns every 2200-pixel request. It permits only one in-flight HD request, leaves at least fifteen seconds between HD starts, replaces queued work when the reader moves to another leaf, and applies a sixty-second cooldown after failure. An HD request that has already started is allowed to complete and is retained even if the reader navigates away. Preview failures receive bounded backoff retries; failed preview and HD entries are removed rather than cached. Visible scan elements receive their `src` only after a retained image has loaded, so a temporary upstream failure produces an explicit loading or retry state rather than a broken-image icon. This policy follows a concurrent five-image trial that produced HTTP `429 Too Many Requests` responses from Gallica and a later sequential-preload trial that was still insufficiently reliable in ordinary use.
