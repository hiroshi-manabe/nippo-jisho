...
The application also retains the exact IIIF JPEG URL in a small in-memory sliding cache of `Image` objects. The current leaf loads at high priority; after it finishes, the two preceding and two following leaves are preloaded sequentially during browser idle time, with a delay between requests. Only one background preload is active at once because a concurrent five-image trial produced HTTP `429 Too Many Requests` responses from Gallica. Failed entries are removed rather than retained, and a current-page failure receives bounded backoff retries plus an explicit retry control. Visible scan elements receive their `src` only after the retained image has loaded, so a temporary upstream failure produces a loading or retry state rather than a broken-image icon. Moving through the dictionary therefore reuses already loaded compressed JPEG resources without maintaining hidden DOM elements or permanently retaining the full corpus. Entries outside the five-leaf window are released. The browser remains responsible for decoding and may discard decoded pixels under memory pressure; this cache is primarily intended to avoid repeated network transfers.
