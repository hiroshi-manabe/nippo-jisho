...
A later cross-page glyph audit showed that one proposed correction had confused caron with tilde and that the same caron had repeatedly been entered as a circumflex. This does not change the recorded timing, but it demonstrates that magnification alone is insufficient for unfamiliar type: the review interface also needs a verified comparison inventory for similar glyphs. See the [Pilot Diacritic Audit](../pilot/diacritic-audit.md).
