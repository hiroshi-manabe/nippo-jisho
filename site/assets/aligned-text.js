...
(() => {
  ...
  const css=document.createElement('style');css.textContent=`
    ...
    .aligned-panel{display:none}.aligned-active>.line-crop{display:none}
    .aligned-active>.aligned-panel{display:block}.aligned-scan{position:relative}
  ...
  function apply(){
    document.querySelectorAll('.aligned-panel').forEach(panel=>{
      ...
      g.setAttribute('transform',`translate(${prefs.horizontal} ${prefs.vertical})`);
      const scale=scan.getBoundingClientRect().width/Number(scan.dataset.width);
      scan.style.marginTop=(8+Math.max(0,-prefs.vertical)*scale)+'px';
      scan.style.marginBottom=(8+Math.max(0,prefs.vertical+prefs.size*.25)*scale)+'px';
    });
  }
  ...
  function controls(list){
    ...
    bar.innerHTML='<label><input type="checkbox" data-setting="enabled">Aligned baseline text</label><details><summary>Display settings</summary><small>Saved in this browser. Text is a baseline snapshot, not your local edits. Uses rectified OCR images; turn off for the ordinary scan/context view.</small></details>';
  ...
  async function mount(){
    ...
    for(const row of data.rows){
      ...
      const panel=document.createElement('div');panel.className='aligned-panel';const scan=document.createElement('div');scan.className='aligned-scan';scan.dataset.width=row.image_width;scan.style.width=(row.image_width/maxWidth*100)+'%';panel.append(scan);
      const img=new Image();img.src=row.image;img.alt=row.id+' rectified OCR crop';scan.append(img);
      ...
      let n=0;for(const run of row.runs)for(const char of run.text){const i=n++,a=row.alignment[i];if(/\s/.test(char))continue;
        ...
        const t=document.createElementNS(ns,'text');t.textContent=char;t.setAttribute('x',(a.left+a.width*(i-start+.5)/(end-start))*row.image_width/100);t.setAttribute('y',35);t.setAttribute('text-anchor','middle');t.setAttribute('font-family','Georgia,serif');t.setAttribute('font-style',run.typeface==='italic'?'italic':'normal');t.setAttribute('fill',a.kind==='uncertain'?'#c04400':'#006bff');g.append(t);
      }
