# Page view: bnf-f0016

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f16.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **A ANTES DO C.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Acami. *Vermelhidão do corpo de fruitas,* |  |
| `c1-l002` | &emsp;*&c. ¶* Faremono nadoni acamiga ſa- |  |
| `c1-l003` | &emsp;ſu. *Fazerſe vermelho o inchaço, ou ou-* |  |
| `c1-l004` | &emsp;*tra couſa.* |  |
| `c1-l005` | Acamidachi, tçu. *Fazerſe vermelho, ou* |  |
| `c1-l006` | &emsp;*aparecer de vermelho, não he muito vſado.* |  |
| `c1-l007` | Acamiuatari, ru, atta. *Aparecer, ou eſtar* |  |
| `c1-l008` | &emsp;*algũa couſa toda de vermelho, como hum* |  |
| `c1-l009` | &emsp;*monte, ou mato no tempo do Outono, ou* |  |
| `c1-l010` | &emsp;*algũa peſſoa toda veſtida de vermelho.* |  |
| `c1-l011` | Acamizzu. *Certa agoa que ſe offerece aos* |  |
| `c1-l012` | &emsp;*Fotoques. vide* Aca. |  |
| `c1-l013` | Acane. *Hũas raizes deruas com que ſe tinge* |  |
| `c1-l014` | &emsp;*de cor vermelha. ¶* Acane momen. |  |
| `c1-l015` | Acane coſode. *Canga, ou coſode tinto des-* |  |
| `c1-l016` | &emsp;*ta cor.* |  |
| `c1-l017` | Acanomizzu. *Vide* Aca. |  |
| `c1-l018` | Acanunaca. *Amizade, ou liança como de* |  |
| `c1-l019` | &emsp;*caſados, ou amigos bẽ unidos, & que não ha* |  |
| `c1-l020` | &emsp;*cauſa por via de deſamor pera ſe apartarẽ.* |  |
| `c1-l021` | Acanu vacare. *Apartamento dos que eſtão* |  |
| `c1-l022` | &emsp;*bem unidos, ou amigos.* |  |
| `c1-l023` | Acarasama. *Adu. Leuemente, ou dẽpres* |  |
| `c1-l024` | &emsp;*ſado, ou aprimeira vez.* |  |
| `c1-l025` | Acarasamana. *Couſa leue como de empreſta-* |  |
| `c1-l026` | &emsp;*do, l, de primeira vez.* |  |
| `c1-l027` | &emsp;Acarasamani. *Adu.* |  |
| `c1-l028` | Acarami, u, òda. *Fazerſe vermelho, ou* |  |
| `c1-l029` | &emsp;*tomar cor ¶* Qinoniga acaramu. *A-* |  |
| `c1-l030` | &emsp;*madurecer a fruita.* |  |
| `c1-l031` | Acari. *Claridade. ¶* Acariye daſu. |  |
| `c1-l032` | &emsp;*Tirar a lume, ou claridade algũa couſa. ¶ Itẽ,* |  |
| `c1-l033` | &emsp;*Per met. Tirar a publico. ¶* Acariye |  |
| `c1-l034` | &emsp;zzuru. *Sair alume algũa couſa, ou* |  |
| `c1-l035` | &emsp;*em publico. ¶* Acariga ſaſu. *Entrar* |  |
| `c1-l036` | &emsp;*a claridade.* |  |
| `c1-l037` | Acari, u, atta. *Dar luz, ou reſplandecer.* |  |
| `c1-l038` | &emsp;*¶ Item, Dar a candea bom lume. ¶* A= |  |
| `c1-l039` | &emsp;carta. *Eſclareceo o tempo que eſtaua tol* |  |
| `c1-l040` | &emsp;*dado, ou chuuoſo.* |  |
| `c1-l041` | Acarisaqi. *Diante da claridade. ¶ Vt,* |  |
| `c1-l042` | &emsp;Acarisaqini tatçu. *Porſe diante da cla=* |  |
| `c1-l043` | &emsp;*ridade, ou impedilla.* |  |
| `c1-l044` | Acaritori. *Freſta do terrado, l, telhado pera* |  |
| `c1-l045` | &emsp;*entrar a claridade* |  |
| `c1-l046` | Acarixǒji. *Adufas feitas de papel pera* |  |
| `c1-l047` | &emsp;*entrar a claridade.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **A ANTES DO C.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Acatçuchi. *Terra vermelha.* |  |
| `c2-l002` | Acatçuqi. *Alua, ou madrugada.* |  |
| `c2-l003` | Acatçuqigata. *Tempo da madrugada, ou* |  |
| `c2-l004` | &emsp;*de antemanhã.* |  |
| `c2-l005` | Acatçuqi ide. *Saida pera fora de caſa an-* |  |
| `c2-l006` | &emsp;*te menhã.* |  |
| `c2-l007` | Acatçuqizzuqi. *Lũa que ſaye antemanhã.* |  |
| `c2-l008` | Acatori. *Balde com que tirão a agoa da bõ-* |  |
| `c2-l009` | &emsp;*ba. ¶ Item, Hũ pano com que cobrem a ſel* |  |
| `c2-l010` | &emsp;*la & ancas do cauallo quãdo nelle ſobem mo-* |  |
| `c2-l011` | &emsp;*lheres. ¶ Item, Eſcouinha de alimpar* |  |
| `c2-l012` | &emsp;*pentem &c.* |  |
| `c2-l013` | Acauotoxi. *Sabão, ou qualquer couſa com-* |  |
| `c2-l014` | &emsp;*que ſe tirão as nodoas, e çujidade.* |  |
| `c2-l015` | Acaxi, ſu, aita. *Aclarar. ¶* Fiuo aca- |  |
| `c2-l016` | &emsp;ſu. *Acender o fogo. ¶* Yǒuo acaſu. |  |
| `c2-l017` | &emsp;*Paſſar anoite ſem dormir ate amanhecer.* |  |
| `c2-l018` | &emsp;*¶* Raſsocuuo acaſu. *Acender a can-* |  |
| `c2-l019` | &emsp;*dea. ¶* Aburafiuo acaſu. *Acender* |  |
| `c2-l020` | &emsp;*candea de azeite. ¶* Fuxinuo acaſu. |  |
| `c2-l021` | &emsp;*Declarar a duuida.* |  |
| `c2-l022` | Acaxicane, uru, eta. *Paſſar anoite, ou tẽ-* |  |
| `c2-l023` | &emsp;*po com dificuldade como eſperando por al-* |  |
| `c2-l024` | &emsp;*guẽm, &c. ¶* Sagui yiqu tçuuji, fi- |  |
| `c2-l025` | &emsp;uomo acaxi cane, curaxi vazzurǒ. *Sõ-* |  |
| `c2-l026` | &emsp;*fir o tempo comprido polla triſteza, & affli-* |  |
| `c2-l027` | &emsp;*ção do animo. S.* |  |
| `c2-l028` | Acaza. *Hũa erua.* |  |
| `c2-l029` | Acazzura. *Roſto vermelho.* |  |
| `c2-l030` | Acazzuqi, u, uita. *Pegarſe a çujidade do* |  |
| `c2-l031` | &emsp;*corpo no veſtido, ou noutra couſa.* |  |
| `c2-l032` | Accô, Varucuchi. *Injuria de palauras, ou* |  |
| `c2-l033` | &emsp;*ruins palauras. ¶ Vt,* Accô iuru. *Mal* |  |
| `c2-l034` | &emsp;*dizer. ¶* Accôuo faqu. *Idem. ¶* Ac- |  |
| `c2-l035` | &emsp;cô zǒgon. *Injuria de palaura.* |  |
| `c2-l036` | Achi. *Adu. Ali, ou là.* |  |
| `c2-l037` | Achi, cochi. *Adu. Decà pera là.* |  |
| `c2-l038` | Acǒ. *Adu. Com claridade, ou de dia.* |  |
| `c2-l039` | &emsp;*¶* Acǒ tçuita. *Ainda cheguei de dia.* |  |
| `c2-l040` | Aco. *Por o Bonzo fogo ſobre o corpo morto q̃* |  |
| `c2-l041` | &emsp;*ſe hade queimar como ceremonia de enterra-* |  |
| `c2-l042` | &emsp;*mento.* |  |
| `c2-l043` | Acoda, l, acodauri. *Hũs melões ama-* |  |
| `c2-l044` | &emsp;*relos, & redondos.* |  |
| `c2-l045` | Acogare, ruru, eta. *Affligirſe muito com* |  |
| `c2-l046` | &emsp;*affecto, ou deſejo dalgũa couſa. ¶* Tçuyi, |  |
| `c2-l047` | &emsp;fanani acogaruru. *Sentir grande apetite,* |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | & |
