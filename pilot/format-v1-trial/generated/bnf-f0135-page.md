# Page view: bnf-f0135

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f135.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO O.** |  |

## Column 1 before internal section heading

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1a-l001` | Cozacurauodoxi. *Certo feitio, ou tecedura* |  |
| `c1a-l002` | &emsp;*de armas.* |  |
| `c1a-l003` | Côzai. *Saber, & Sagacidade. Vt,* Cô- |  |
| `c1a-l004` | &emsp;zainafito. *Homem ſagaz, & eſperto.* |  |
| `c1a-l005` | Cozaixǒ. *Certa dignidade de homẽs, & mo-* |  |
| `c1a-l006` | &emsp;*lheres que ſeruem na caſa do Dairi*. |  |
| `c1a-l007` | Cǒzan. Tacaiyama. *Monte alto.* |  |
| `c1a-l008` | Cozaſa. *Certa layade erua como caninhas curtas.* |  |
| `c1a-l009` | Cozauaxi. *Hum genero de figos de Iapão.* |  |
| `c1a-l010` | Cozaxiqi. *Caſa, ou camara pequena.* |  |
| `c1a-l011` | Cozǒ. Curacura. *Gudão, ou logea onde ſe* |  |
| `c1a-l012` | &emsp;*guarda fato, &c.* |  |
| `c1a-l013` | Cozô. *Menino que ſe cria na Tera rapado.* |  |
| `c1a-l014` | Cozo. *Anno paſſado.* |  |
| `c1a-l015` | Cozori, u, otta. *Ajuntarſe como em pinha,* |  |
| `c1a-l016` | &emsp;*& mui apertadamente muita gente, &c.* |  |
| `c1a-l017` | Cozorifa. *Hũa certa alabarda pequena.* |  |
| `c1a-l018` | Cozǒxi. *Molheres baixas que ſeruem em caſa* |  |
| `c1a-l019` | &emsp;*do Dajri*. |  |
| `c1a-l020` | Côzui. Vouoi mizzu. *Grande enchente, ou* |  |
| `c1a-l021` | &emsp;*diluuio dagua.* |  |
| `c1a-l022` | Cozuye. *Raminhos do alto da aruore.* |  |
| `c1a-l023` | Côzzu. *Cagado.* |  |
| `c1a-l024` | Cozzuchi. *Martelinho.* |  |
| `c1a-l025` | Cozzumi. *Palha q̃ eſtà em feixada, & jũta.* |  |
| `c1a-l026` | Cozzumi, u, unda. *Acumular, ou pór hũas* |  |
| `c1a-l027` | &emsp;*couſas ſobre outras. ¶ Item, Cair o caua-* |  |
| `c1a-l028` | &emsp;*lo embicando.* |  |
| `c1a-l029` | Cozzuna. *Corda, ou calaure pequeno.* |  |
| `c1a-l030` | Cozzuqe. Xiru, *& arroz em eſcudelinhas,* |  |
| `c1a-l031` | &emsp;*he comer leue feito depreſſa pera algũ hoſpede.* |  |
| `c1a-l032` | Cozzutai, ǒ, ǒta. *Andar o bugio pellos ra-* |  |
| `c1a-l033` | &emsp;*mos das aruores. Vt,* Mineni cozzutǒ ſa- |  |
| `c1a-l034` | &emsp;runo coye. *Voz do bugio que anda corren-* |  |
| `c1a-l035` | &emsp;*do pellas aruores dos montes.* |  |

## Column 1 internal section heading

| Physical line | Main position | Far right |
| --- | --- | --- |
| `s1-l001` | **C ANTES DO V.** |  |

## Column 1 after internal section heading

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1b-l001` | CV. Curuximi. *Pena, ou tormento. Vt,* |  |
| `c1b-l002` | &emsp;Cuuo vquru. *Receber tormentos.* |  |
| `c1b-l003` | Cu. *Sentença, ou dito.* |  |
| `c1b-l004` | Cù. Bup*. Hũa couſa imperfectißima, &* |  |
| `c1b-l005` | &emsp;*de minimo ſer como materia prima, ou vacuo.* |  |
| `c1b-l006` | Cubari, u, atta. *Repartir. ¶* Meuo cu |  |
| `c1b-l007` | &emsp;baru. *Derramar os olhos por diuerſas partes.* |  |
| `c1b-l008` | Cubari, u, atta. *Estar a lenha posta no fogo* |  |
| `c1b-l009` | &emsp;*pera arder.* |  |
| `c1b-l010` | Cube, uru, eta. *Por, ou meter lenha, ou* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO O.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `p-l001` | **62.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | &emsp;*qualquer outra couſa no fogo. Vt,* Funi taqi- |  |
| `c2-l002` | &emsp;guiuo cuburu. *Meter, ou por lenha no fogo.* |  |
| `c2-l003` | Cubeſoye, uru, eta. *Accrecentar lenha à* |  |
| `c2-l004` | &emsp;*que eſtà ardendo.* |  |
| `c2-l005` | Cubetçugui, u, tçuida. *Idem.* |  |
| `c2-l006` | Cubi. *Peſcoço. ¶* Cubiuo toru. *Tomar* |  |
| `c2-l007` | &emsp;*cabeças naguerra. ¶* Cubiuo cucuru, l, |  |
| `c2-l008` | &emsp;cubiru. *Enforcarſe. ¶* Cubiuo qiru, |  |
| `c2-l009` | &emsp;vtçu, cacu, l, fanuru, *l*, vchi fanaſu, *l*, |  |
| `c2-l010` | &emsp;votoru. *Cortar a cabeça. ¶* Cubiuo ca- |  |
| `c2-l011` | &emsp;quru. *Dependurar a cabeça, ou polla em* |  |
| `c2-l012` | &emsp;*alto. ¶ Item,* Cubiuo caquru. *A-* |  |
| `c2-l013` | &emsp;*poſtar pondo a cabeça.* |  |
| `c2-l014` | Cubicaqigatana. *Eſpada de cortar cabeças.* |  |
| `c2-l015` | Cubicaxe. *Iugo dos bois, ou com que ator-* |  |
| `c2-l016` | &emsp;*mentão, & ſojugão homens malfeitores.* |  |
| `c2-l017` | Cubimaqi. *Pano como* Vataboxi *Pera cu* |  |
| `c2-l018` | &emsp;*brir o peſcoço.* |  |
| `c2-l019` | Cubiri, ru, itta. *Atar, ou amarrar. ¶ Itẽ,* |  |
| `c2-l020` | &emsp;*Enforcarſe. Vt,* Cubiuo cubiru. x. |  |
| `c2-l021` | Cubitama. *Collar, ou colleira que poem no* |  |
| `c2-l022` | &emsp;*peſcoço dos cães, ou gatos.* |  |
| `c2-l023` | Cubixen. *Dinheiro que ſedà pera eſcapar da* |  |
| `c2-l024` | &emsp;*morte. Mas não he palaura antiga, & propria.* |  |
| `c2-l025` | Cubǒ. Xǒgunno coto. *Dignidade de capi-* |  |
| `c2-l026` | &emsp;*tão principal, ou general de todo Iapão.* |  |
| `c2-l027` | Cubocana. *Couſa concaua.* |  |
| `c2-l028` | Cuboi. *Couſa concaua, ou funda. ¶* I*tem,* |  |
| `c2-l029` | &emsp;*Permet. Somenos, ou inferior nas letras, ou* |  |
| `c2-l030` | &emsp;*em outra arte.* |  |
| `c2-l031` | &emsp;Cuboſa. |  |
| `c2-l032` | &emsp;Cubô. |  |
| `c2-l033` | Cubome, uru, eta. *Fazer concauo. ¶* Te |  |
| `c2-l034` | &emsp;uo cubomuru. *Encouar a palma da mão,* |  |
| `c2-l035` | &emsp;*ou fazer cõcaua pera tomar algũa couſa nella.* |  |
| `c2-l036` | Cubomi, u, ôda. *Fazerſe concauo, ou fundo.* |  |
| `c2-l037` | Cùbucu. Munaxij fara. i. Suqi fara. *Barri-* |  |
| `c2-l038` | &emsp;*ga vazia. i. Eſtar em jejum.* |  |
| `c2-l039` | Cubuqi. *Saco de palha.* |  |
| `c2-l040` | Cucai. Curuximino vmi. *Mar de tormen-* |  |
| `c2-l041` | &emsp;*tos. i. Mundo.* |  |
| `c2-l042` | Cucchù. Anano uchi. *Dentro na coua. Vt,* |  |
| `c2-l043` | &emsp;Xixi cucchùni ijù naxi. Xix*. Eſtando o* |  |
| `c2-l044` | &emsp;*lião na ſua coua, nenhum outro animal ouſa* |  |
| `c2-l045` | &emsp;*entrar.* |  |
| `c2-l046` | Cuchi. *Boca. ¶* Cuchiga vǒ. *Concor-* |  |
| `c2-l047` | &emsp;*dar no que ſe diz. ¶* Cuchi namezzu- |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `cw-l001` | **riuo** |  |

## Printed signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `s-l001` | **Q 2** |  |
