# Page view: bnf-f0082

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f82.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO A.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Canatçugui. *Fiſga de ferro.* |  |
| `c1-l002` | Canatebǒ. *Bordão de ferro.* |  |
| `c1-l003` | Canatoco. *Safra, ou bigorna de ferreiro.* |  |
| `c1-l004` | Canatôdai. *Caſtiçal alto de ferro, ou outro* |  |
| `c1-l005` | &emsp;*metal.* |  |
| `c1-l006` | Canaua. *Trempe de ferro.* |  |
| `c1-l007` | Canaxigueni. *Adu. Com triſteza, ou triſ-* |  |
| `c1-l008` | &emsp;*temente.* |  |
| `c1-l009` | Canaxij. *Couſa triſte.* |  |
| `c1-l010` | &emsp;Canaxifa. |  |
| `c1-l011` | &emsp;Canaxu. |  |
| `c1-l012` | Canaximi. *Triſteza. ¶* Canaximiuo ca- |  |
| `c1-l013` | &emsp;quru. *Dar triſteza. ¶* Canaximiuo |  |
| `c1-l014` | &emsp;moyouoſu. *Entriſtecerſe muito. ¶* Cana- |  |
| `c1-l015` | &emsp;ximi qimoni meizuru. *Penetrar a triſteza* |  |
| `c1-l016` | &emsp;*muito a alguem. ¶* Canaximimini ama- |  |
| `c1-l017` | &emsp;ru. *Ter grandiſſima triſteza.* |  |
| `c1-l018` | Canaximi, u, uda. *Entriſtecerſe.* |  |
| `c1-l019` | Canaxiqi. *Safra, ou bigorna de ferrei-* |  |
| `c1-l020` | &emsp;*ro. B.* |  |
| `c1-l021` | Canaxiuo. *Tempera de ferro.* |  |
| `c1-l022` | Canayaguen. *Almofaris de Iapão como na-* |  |
| `c1-l023` | &emsp;*ueta.* |  |
| `c1-l024` | Canayama. *Minas de metal.* |  |
| `c1-l025` | Canaye, uru, eta. *Comprir como deſejo, &c.* |  |
| `c1-l026` | &emsp;*Vt,* Nozomiuo canayuru. *Comprir os de-* |  |
| `c1-l027` | &emsp;*ſejos dalguem. ¶* Xoxôuo canayuru. *Cõ-* |  |
| `c1-l028` | &emsp;*prir a petição de alguem.* |  |
| `c1-l029` | Canazzuchi. *Martelo de ferro.* |  |
| `c1-l030` | Canba. Axe vma. i. Axe caita vma. *Caua-* |  |
| `c1-l031` | &emsp;*lo cançado, & ſuado. ¶* Canbauo yaſu- |  |
| `c1-l032` | &emsp;muru. *Fazer deſcançar o caualo ſuado.* |  |
| `c1-l033` | Canbachi. *Grande ſeca. i.* Canbat. |  |
| `c1-l034` | Canban. Fuda ita. *Taboa vruxada, ou pa-* |  |
| `c1-l035` | &emsp;*pel em que ſe eſcreue, & borra facilmente* |  |
| `c1-l036` | &emsp;*ſem ficar ſinal.* |  |
| `c1-l037` | Canbara. *Nome de hum lugar que ha no rei-* |  |
| `c1-l038` | &emsp;*no de Yechijen. ¶ Item, Hũas candeas q̃* |  |
| `c1-l039` | &emsp;*vem deſte lugar. ¶* Canbarayǒji. *Paos de* |  |
| `c1-l040` | &emsp;*dentes de ſalgueiro que vem deſte lugar.* |  |
| `c1-l041` | Canbat. i. Ficeri. *Seca.* |  |
| `c1-l042` | Canbaxe. *Reſſo.* |  |
| `c1-l043` | Canbaxij. *Couſa cheiroſa.* |  |
| `c1-l044` | &emsp;Canbaxifa. |  |
| `c1-l045` | &emsp;Canbaxu. |  |
| `c1-l046` | Canben. Cangaye vagimayuru. *Conſide-* |  |
| `c1-l047` | &emsp;*ração.* |  |
| `c1-l048` | Canbeǒ, l, cabeǒ. *O curar doenças.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO A.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Canbǒ. i. Bǒzo. *Que tem cuidado dalgũa* |  |
| `c2-l002` | &emsp;*terra.* |  |
| `c2-l003` | Canbô. Cadamaxij. Facaricoto. i. *Ar-* |  |
| `c2-l004` | &emsp;*dil, ou engano pera fazer mal a alguem.* |  |
| `c2-l005` | Canbocu. Fude ſumi. *Pena, & tinta.* |  |
| `c2-l006` | &emsp;*Item, Algũa vez ſe toma por carta.* |  |
| `c2-l007` | Canbun. *Cartas ou eſcritos da china de boa no-* |  |
| `c2-l008` | &emsp;*ta, eſtillo, &c.* |  |
| `c2-l009` | Canbut. Foxi mono. *Couſa ſeca como peixe,* |  |
| `c2-l010` | &emsp;*mariſco, abobora, &c.* |  |
| `c2-l011` | Canca. Canno iye. *China.* |  |
| `c2-l012` | Canca. Xizzucana ſima. *Eſtar deſocupado.* |  |
| `c2-l013` | Canchicu. Carano taqe. *Canas da China, de* |  |
| `c2-l014` | &emsp;*que ſe fazem frautas.* |  |
| `c2-l015` | Canchicuno yôgiô. *Frauta aſſi chamada.* |  |
| `c2-l016` | Canchô. *Reino da china.* |  |
| `c2-l017` | Canchù. Sumui naca. *Nome do frio. A* |  |
| `c2-l018` | &emsp;*propria palaura que corre he Cangiù.* |  |
| `c2-l019` | Canchùren. Facqeno coroba. *Certa manei-* |  |
| `c2-l020` | &emsp;*ra de ver, & cotejar hũas riſcas, ou linhas* |  |
| `c2-l021` | &emsp;*por onde conjeiturão os que a diuinham em* |  |
| `c2-l022` | &emsp;*Iapão.* |  |
| `c2-l023` | Cancocu. Samui cuni. *Reino frio.* |  |
| `c2-l024` | Cancu. i. Canni curuximu. *Tormento de* |  |
| `c2-l025` | &emsp;*frio. ¶* Cancuni xemerauru. *Ser a-* |  |
| `c2-l026` | &emsp;*tormentado do frio.* |  |
| `c2-l027` | Cancu. Anaxi nigaxi. *Doce, & amargo.* |  |
| `c2-l028` | Cancuchô. *Hum paſſaro q̃ de noite ſe queixa, &* |  |
| `c2-l029` | &emsp;*magoa com frio, & polla manhã em vendo o* |  |
| `c2-l030` | &emsp;*ſol ſe eſquece.* |  |
| `c2-l031` | Canda. *Torto, ou cego de hum olho.* |  |
| `c2-l032` | Candan. Samuxi, atçuxi. *Frio, &* |  |
| `c2-l033` | &emsp;*quẽte.* |  |
| `c2-l034` | Candan. Xizzucani cataru. *Praticar de va-* |  |
| `c2-l035` | &emsp;*gar & à vontade. vt,* Candanuo toguu. |  |
| `c2-l036` | &emsp;*Idem. G.* |  |
| `c2-l037` | Canden. Xizzucana iye. *Caſa ſolitaria.* |  |
| `c2-l038` | Candǒ. Cangaye ataru. i. Fitono qin chi- |  |
| `c2-l039` | &emsp;gǒ. *Quebrar com alguem na amizade, ou* |  |
| `c2-l040` | &emsp;*ficar em deſgraça. Vt,* Voyano can- |  |
| `c2-l041` | &emsp;dǒuo cǒmuru. *Cair em deſgraça do pai.* |  |
| `c2-l042` | Candǒ. i. Nuguijirono mon. *Lauor com oro-* |  |
| `c2-l043` | &emsp;*ſa, ou ſelbagẽ de peça tecida de fio verme-* |  |
| `c2-l044` | &emsp;*lho, & branco.* |  |
| `c2-l045` | Candǒ. Xizzucana michi. i. Buji. *Paz.* |  |
| `c2-l046` | Candǒ. V. gi michi. i. Seqinmichi. *Ata-* |  |
| `c2-l047` | &emsp;*lho, ou caminho mais breue.* |  |
| `c2-l048` | Cando. *China.* |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | Can- |
