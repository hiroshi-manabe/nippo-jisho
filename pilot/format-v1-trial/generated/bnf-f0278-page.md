# Page view: bnf-f0278

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f278.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

Lexical coverage was cross-checked against the supplied Entry Words Data of Nippojisho, NINJAL, Hideyuki Ohshima and Taichi Aida, version 202510 (CC BY 4.0). This is a lexical aid, not a substitute transcription; the supplied BnF scan remains the authority. Source: https://www2.ninjal.ac.jp/textdb_dataset/en/nipp/index.html ; license: https://creativecommons.org/licenses/by/4.0/ .

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **I ANTES DO X.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | &emsp;*i, toda a fazenda, ou herança. ¶* Ixxe- |  |
| `c1-l002` | &emsp;qini fanaruru. l, ixxeqiuo mexi fanaſa- |  |
| `c1-l003` | &emsp;ruru. *Ser priuado de todos ſeus bens aßi mo-* |  |
| `c1-l004` | &emsp;*uis, como de raiz. ¶* Ixxeqiuo yuzzuru. |  |
| `c1-l005` | &emsp;*Dei xar a alguem todos ſeus bens.* |  |
| `c1-l006` | Ixxeqi. Fitotçuno muxiro. *Hũa eſteira.* |  |
| `c1-l007` | Ixxet. i, iſſai. *Adu. Denehũa maneira quã-* |  |
| `c1-l008` | &emsp;*do ſe ajunta com negatiuo. Vt,* Conofoni- |  |
| `c1-l009` | &emsp;uoite ixxet bechiguiua gozanai. *Emmim* |  |
| `c1-l010` | &emsp;*naõ hà outra couſa,* l*, mudança. ¶ Item,* |  |
| `c1-l011` | &emsp;*totalmente, ou em todo caſo.* |  |
| `c1-l012` | Ixxet. *Hũa noua. ¶* Ixxetga qicoyeta. |  |
| `c1-l013` | &emsp;*Sòa hũa noua.* |  |
| `c1-l014` | Ixxi. Fitorigo. *Filho vnico.* |  |
| `c1-l015` | Ixxi. Io fitoſugi. *Hũa linha, l, fio.* |  |
| `c1-l016` | Ixxi. Fitoyeda. *Hum ramo.* |  |
| `c1-l017` | Ixxi. Fitotçuno yubi. *Hum dedo.* |  |
| `c1-l018` | Ixxi. Cami ichimai. *Hũa folha de papel.* |  |
| `c1-l019` | &emsp;*¶* Ixxi fanxẽno jifiuomo fodocoxe. *A-* |  |
| `c1-l020` | &emsp;*inda que ſeja couſa muito pouca como hũa folha* |  |
| `c1-l021` | &emsp;*de papel, ou mea caxa não deixeis de dar* |  |
| `c1-l022` | &emsp;*eſmola.* |  |
| `c1-l023` | Ixxi. *Hum papel eſcrito, como conhecimento,* |  |
| `c1-l024` | &emsp;*&c. ¶* Ixxiuo toru, l, daſu. *Tomar,* |  |
| `c1-l025` | &emsp;*ou dar eſte papel como conhecimento.* |  |
| `c1-l026` | Ixxi. Fitotabi xiſuru. *Hũa morte, ou mor* |  |
| `c1-l027` | &emsp;*rerhũa vez. Vt,* Ixxi ſaiquat xezu. *O q̃* |  |
| `c1-l028` | &emsp;*morre hũa vez não torna a viuer, ou reſuſcitar* |  |
| `c1-l029` | &emsp;*naturalmente.* |  |
| `c1-l030` | Ixxin. Fitorino voya. *Hum pay, ou mãy.* |  |
| `c1-l031` | Ixxin. Fitotçuno mi. *Hum corpo, ou ſubſtã-* |  |
| `c1-l032` | &emsp;*cia, ou hum ſoo. Vt,* Sonoaidani ixxin |  |
| `c1-l033` | &emsp;cacureanauozo foritari qeru. Taif*. Lib.* |  |
| `c1-l034` | &emsp;*32*. *Entre tanto elle ſò fez hũa coua em que* |  |
| `c1-l035` | &emsp;*ſe eſcõdeße. ¶* Ixxindeua narumai. *Eu* |  |
| `c1-l036` | &emsp;*ſoo por mi não poderei.* |  |
| `c1-l037` | Ixxin. Fitotçuno macura. *Hũa almofada. S.* |  |
| `c1-l038` | Ixxin. Fitotçuno cocoro. *Hum coração.* |  |
| `c1-l039` | &emsp;*¶* Ixxin iccǒni negǒ. *Deſejar de coração* |  |
| `c1-l040` | &emsp;*inteiramente ſem ſe diuertir em outras couſas.* |  |
| `c1-l041` | Ixxinpuran.l, ixxinfuran. *Atenção grande,* |  |
| `c1-l042` | &emsp;*& deuação, com que hum lee algum liuro, re-* |  |
| `c1-l043` | &emsp;*za, &c.* |  |
| `c1-l044` | Ixxinpuranni, *Adu. Atenta, & deuo-* |  |
| `c1-l045` | &emsp;*tamente. ¶* Ixxinpuranno teide atta. |  |
| `c1-l046` | &emsp;*Moſtraua muita deuação, & atenção.* |  |
| `c1-l047` | Ixxiqi. Fitotçuno iro. *Hũa laya de couſas.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **I ANTES DO X.** |  |

## Column 2, part 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Ixxiqi. *O entender, ou fazer ſoo. Vt,* Ix- |  |
| `c2-l002` | &emsp;xiqini ſuru. *Fazer, ou menear algũa cou-* |  |
| `c2-l003` | &emsp;*ſa à ſua vontade. ¶* Ixxiqini tocorouo |  |
| `c2-l004` | &emsp;xiru. *Gouernar por ſi ſoo algum lugar.* |  |
| `c2-l005` | Ixxit. Fitotçu vxinǒ. *Hũa perda. Vt,* |  |
| `c2-l006` | &emsp;Chixaua xenrioni xite canarazu ixxit ari; |  |
| `c2-l007` | &emsp;guxaua xenrioni xite canarazu ittocu ari. |  |
| `c2-l008` | &emsp;Xix. *O prudente, & ſabio de mil couſas que* |  |
| `c2-l009` | &emsp;*conſidera hũa ſoo erra, o ignorante de mil hũa* |  |
| `c2-l010` | &emsp;*ſoo a certa.* |  |
| `c2-l011` | Ixxǒ. Fitotçuno naricuxe. *Hũa nature-* |  |
| `c2-l012` | &emsp;*za.* Bup. |  |
| `c2-l013` | Ixxô. Fitotçuno xôco. *Hum teſtemunho.* |  |
| `c2-l014` | Ixxǒ. *Toda a vida. Vt,* Ixxǒ fubon. *Ca-* |  |
| `c2-l015` | &emsp;*ſtidade perpetua, ou de toda a vida. ¶ Itẽ,* |  |
| `c2-l016` | &emsp;*Hum meſmo nacimento. Vt,* Ippucu ix- |  |
| `c2-l017` | &emsp;xǒno qiǒdai. *Irmãos nacidos do meſmo* |  |
| `c2-l018` | &emsp;*pay, & mãy.* |  |
| `c2-l019` | Ixxǒ. Fitotçu toco. *Hum leito, ou catre.* |  |
| `c2-l020` | Ixxǒ. Fitotçuno vgi. *Hũa familia, ou gè-* |  |
| `c2-l021` | &emsp;*ração.* |  |
| `c2-l022` | Ixxô. *Vt,* Ixxô ichinen. *O inuocar inten* |  |
| `c2-l023` | &emsp;*ſa, & atentamente o nome do Fotoque*. Bup. |  |
| `c2-l024` | Ixxô. *Hum riſo. S.* |  |
| `c2-l025` | Ixxǒ. Fitocoye. *Hũa vez.* |  |
| `c2-l026` | Ixxo. Fitotocoro. *Hum lugar. Vt,* Ixxo |  |
| `c2-l027` | &emsp;fugiǔno ſô. *Bonzo que ſempre anda pere-* |  |
| `c2-l028` | &emsp;*grinando ſem caſa certa.* |  |
| `c2-l029` | Ixxo. Fitotçu caqu. *Hũa carta. ¶* Ixxo |  |
| `c2-l030` | &emsp;uo mochij ſoro. *Eſcreuo esta carta. No* |  |
| `c2-l031` | &emsp;*Cami ſe diz,* Ixxouo motte mǒxi ſoro. *S.* |  |
| `c2-l032` | Ixxo fugiǔ. *Não morar em hum lugar certo.* |  |
| `c2-l033` | &emsp;*Vide,* Ixxo. |  |
| `c2-l034` | Ixxǒgai. i, Ixxeno aida. *Toda a vida.* |  |
| `c2-l035` | Ixxǒzocu. *Hum corpo darmas inteiro, ou eſ* |  |
| `c2-l036` | &emsp;*quipação de veſtidos, bons ornamentos, &c.* |  |
| `c2-l037` | Ixxǔ. Fitotçuno xǔ. *Hũa ſeita.* |  |
| `c2-l038` | Ixxucu. Fitotomari. *Pouſada de hũa noite.* |  |
| `c2-l039` | &emsp;*Vt,* Ixxucu ſuru. *Pouſar, ou agaſalhar ſe* |  |
| `c2-l040` | &emsp;*hũa noite.* |  |
| `c2-l041` | Ixxucu. i, Ixxǒzocu. *Hum corpo darmas* |  |
| `c2-l042` | &emsp;*inteiro. ¶* Ixxucu yorô. *Armarſe todo* |  |
| `c2-l043` | &emsp;*de ponto em branco.* |  |

## Column 2 internal heading

| Physical line | Main position | Far right |
| --- | --- | --- |
| `s2-01-l001` | **I ANTES DO Y.** |  |

## Column 2, part 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l044` | IY. Mexi. *Arroz cozido.* |  |
| `c2-l045` | &emsp;Iy, yǔ, yǔta. *Falar, ou dizer.* |  |
| `c2-l046` | Iya. *Não.* |  |

## Bottom-right catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `cw-l001` |  | Iya. |
