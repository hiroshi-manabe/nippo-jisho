# Page view: bnf-f0097

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f97.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO A.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Catarigoto. *Pratica.* |  |
| `c1-l002` | Catari idaxi, ſu, aita. *Começar a praticar,* |  |
| `c1-l003` | &emsp;*ou falar couſas paſſadas. ¶* Cono fito |  |
| `c1-l004` | &emsp;qiuoyete iroirono cotouo catariidaita. |  |
| `c1-l005` | &emsp;Mon*. Eſte homem tomando alento contou* |  |
| `c1-l006` | &emsp;*varias couſas. (apratica.* |  |
| `c1-l007` | Catariſute, tçuru, eta. *Deixar, ou acabar* |  |
| `c1-l008` | Cataritçurane, uru, eta. *Leuar ẽfiada apratica.* |  |
| `c1-l009` | Cataritçutaye, uru, eta. *Praticar algũa* |  |
| `c1-l010` | &emsp;*couſa por deſcendencia como de pais a filhos,* |  |
| `c1-l011` | &emsp;*& filhos a netos, &c.* |  |
| `c1-l012` | Cataritçuzzuqe, uru, eta. *Praticar en* |  |
| `c1-l013` | &emsp;*fiado. Vt,* Coxi cata yucuſuyeno co- |  |
| `c1-l014` | &emsp;todomouo cataritçuzzuqete ſamezame- |  |
| `c1-l015` | &emsp;to nacareta. Fei*. L. 4. Continuando* |  |
| `c1-l016` | &emsp;*apratica do que tinha paſſado, & do que a-* |  |
| `c1-l017` | &emsp;*uia de acontecer chorou muitas lagrimas.* |  |
| `c1-l018` | Catariuocoxi, ſu, oita. *Tornar apraticar* |  |
| `c1-l019` | &emsp;*o paſſado.* |  |
| `c1-l020` | Cataſagari. *O estar hũa parte mais bai*x*a que* |  |
| `c1-l021` | &emsp;*outra. ¶ Item, Eſtar o veſtido de hũa* |  |
| `c1-l022` | &emsp;*parte mais baixo, ou comprido.* |  |
| `c1-l023` | Cataſagarina. *Couſa deſigual por eſtar hũa* |  |
| `c1-l024` | &emsp;*parte mais baixa que outra.* |  |
| `c1-l025` | Cataſaqi. *Hũa ponta.* |  |
| `c1-l026` | Cataſode. *Hũa manga.* |  |
| `c1-l027` | Cataſoguizzucuri. *Certo modo de fazer ca-* |  |
| `c1-l028` | &emsp;*ſas toſca, & imperfeitamente.* |  |
| `c1-l029` | Catatçu. *Cuſpinho. Mas não ſe vſa ſe não* |  |
| `c1-l030` | &emsp;*desta maneira. Vt,* Catatçuuo nomu. |  |
| `c1-l031` | &emsp;*Estar em grãde trabalho, & perigo* X*. Mais* |  |
| `c1-l032` | &emsp;*propriamente ſediz de quẽ eſta cõcuidado, &* |  |
| `c1-l033` | &emsp;*afflição ſobre algũ caſo.* |  |
| `c1-l034` | Catatçuburi, l, quaguiù. *Caracol.* |  |
| `c1-l035` | Catatçuqi. *Boyãozinho em que ſe mete* Cha |  |
| `c1-l036` | &emsp;*moido pera ſe gaſtar.* |  |
| `c1-l037` | Catate. *Hũa mão, frecha, &c.* |  |
| `c1-l038` | Catatoqi. i. Fanji, l, fantôqi. *Por hum* |  |
| `c1-l039` | &emsp;*pouco, ou mea hora.* |  |
| `c1-l040` | Cataua. *Aleijão. ¶* Catauani naru. *Ser* |  |
| `c1-l041` | &emsp;*aleijado.* |  |
| `c1-l042` | Catauaguruma. *Carro como aleijado de hũa ſò* |  |
| `c1-l043` | &emsp;*roda que não presta ja para nada. Acomoda* |  |
| `c1-l044` | &emsp;*ſe tambẽ nos* Mais *a hum homem que nenhum* |  |
| `c1-l045` | &emsp;*remedio humano tem, &c.* |  |
| `c1-l046` | Catauana. *Peſſoa aleijada.* |  |
| `c1-l047` | Catauaqi. *Hũa ilharga, ou lado.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO A.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `pn-l001` | 43. |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Catauara. *A hũa parte, ou jũto dalgũa couſa.* |  |
| `c2-l002` | &emsp;*Vt,* Michino catauara. *Borda do caminho.* |  |
| `c2-l003` | Catauazzuqi, u, uita. *Fazerſe aleijado.* |  |
| `c2-l004` | &emsp;*Vſaſe commummente no preterito.* |  |
| `c2-l005` | Catauoca. *P*. *Hũa parte, ou banda do ou-* |  |
| `c2-l006` | &emsp;*teiro.* |  |
| `c2-l007` | Catauomoi. *P*. *Amor de hũa ſò parte.* |  |
| `c2-l008` | Catauomote. *Hũa pagina.* |  |
| `c2-l009` | Catauonami. *P*. *Ondas que não deixão raſ-* |  |
| `c2-l010` | &emsp;*to, ou ſinal como as que dão em rocha que não* |  |
| `c2-l011` | &emsp;*tem praya.* |  |
| `c2-l012` | Catauoxi. *Vt,* Catauoxini ſuru. *Dar, ou* |  |
| `c2-l013` | &emsp;*combater com gente de hũa ſò parte ſem auer* |  |
| `c2-l014` | &emsp;*outra de que ſeja rebatida. ¶ Item, Por* |  |
| `c2-l015` | &emsp;*pezo de hũa parte, & não de outra pera car-* |  |
| `c2-l016` | &emsp;*regar, ou eſpremer algũa couſa.* |  |
| `c2-l017` | Catauoxina. *Parte que foi ſò carregada, ou* |  |
| `c2-l018` | &emsp;*eſpremida com algum pezo, & a outra não.* |  |
| `c2-l019` | Cataxigataxi. *Hũa de duas couſas que ſe contão* |  |
| `c2-l020` | &emsp;*por pares como çapato quando diſcrepa. i.* |  |
| `c2-l021` | &emsp;*não he irmão do outro. Vt,* Cataxigataxi |  |
| `c2-l022` | &emsp;ſaqu. *Calçar hũçapato meu & outro alheo, ou* |  |
| `c2-l023` | &emsp;*de outra laya. He palaura baixa, & quãdo* |  |
| `c2-l024` | &emsp;*muito ſe vſara em çapatos. A propria* |  |
| `c2-l025` | &emsp;*palaura, & mais gèral em couſas de pares,* |  |
| `c2-l026` | &emsp;*como* Tabis, biǒbus *&c. he* Catachigu. |  |
| `c2-l027` | Cataxiqi, u, ijta. *Eſtender hũa ſò manga,* |  |
| `c2-l028` | &emsp;*&c. Vt,* Sodeuo cataxijte nuru. |  |
| `c2-l029` | &emsp;*Estendendo a manga dormir ſobre ella, ou dor-* |  |
| `c2-l030` | &emsp;*mir ſobre o braço, ou eſtribado ſobre o coto* |  |
| `c2-l031` | &emsp;*uelo.* |  |
| `c2-l032` | Cataya. Sumǒuo toru ba. T*erreiro onde* |  |
| `c2-l033` | &emsp;*ſe luta.* |  |
| `c2-l034` | Catayamazato. *Pouoação dos montes.* |  |
| `c2-l035` | Cataye. i. Vaquno yeda. *ramo de hua parte.* |  |
| `c2-l036` | Cataye. *Vt,* Cataye nomono. *Gente, ou* |  |
| `c2-l037` | &emsp;*peſſoas de ſeruiço que não ſão das mais altas nẽ* |  |
| `c2-l038` | &emsp;*das mais baixas. ¶* Catayeno nhôbo. |  |
| `c2-l039` | &emsp;*Molheres de ſeruiço das ordinarias que não ſão* |  |
| `c2-l040` | &emsp;*muito baixas.* |  |
| `c2-l041` | Catayori, u, otta. *Chegarſe pera hũa parte.* |  |
| `c2-l042` | Catazzuri. *Deſigualdade de hũa parte da balan* |  |
| `c2-l043` | &emsp;*ça, ou de outra couſa. ¶* Catazzuriga |  |
| `c2-l044` | &emsp;aruca miyo. *Vede ſe ha deſigualdade, quer ſe* |  |
| `c2-l045` | &emsp;*ja de pezo, quer ſeja entre dous que jogão, ou* |  |
| `c2-l046` | &emsp;*ſão deſiguais nalgũa arte, &c.* |  |
| `c2-l047` | Catazurina. *Couſa deſigual como leue de hũa* |  |

## Printed signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `sig-l001` | L 3 |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | parte |
