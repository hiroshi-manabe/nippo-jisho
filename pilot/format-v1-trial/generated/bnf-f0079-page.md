# Page view: bnf-f0079

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f79.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO A.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Caitei. Vmino ſoco. *Profundo do mar.* |  |
| `c1-l002` | &emsp;*¶* Caiteini xizzumu. *Ser metido, ou mer* |  |
| `c1-l003` | &emsp;*gulharſe no fundo do mar.* |  |
| `c1-l004` | Caitori. *Paſſaro que ſe cria, & domeſtica na* |  |
| `c1-l005` | &emsp;*gayola, &c.* |  |
| `c1-l006` | Caitori, u, otta. *Tomar, ou aleuantar. Vt,* |  |
| `c1-l007` | &emsp;Yxǒno fuſouo caitoru. *Aleuantar as fral* |  |
| `c1-l008` | &emsp;*das do veſtido.* |  |
| `c1-l009` | Caitori, u, otta. *Comprar, ou tomar com-* |  |
| `c1-l010` | &emsp;*prando. Vt,* Funeuo caitotte cogui cuda |  |
| `c1-l011` | &emsp;rǒ. *Comprando hũa embarcação decerei re-* |  |
| `c1-l012` | &emsp;*mando.* |  |
| `c1-l013` | Caiuoqi, u, oita. *Comprando guardar.* |  |
| `c1-l014` | Caiuoqi, uru, qita. *Leuantarſe o que eſtà* |  |
| `c1-l015` | &emsp;*deitado.* |  |
| `c1-l016` | Caixacu. *O ajudar a morrer cortando a cabeça* |  |
| `c1-l017` | &emsp;*ao que ſe cortou a barriga. ¶ Item, Aju-* |  |
| `c1-l018` | &emsp;*da q̃ dà algũ homẽ como ayo a algũ menino no* |  |
| `c1-l019` | &emsp;*bre tendo cuidado delle, encaminhandoo, &c.* |  |
| `c1-l020` | Caixacunin. *O que ajuda a morrer a outro cor* |  |
| `c1-l021` | &emsp;*tando lhe a cabeça. ¶ Item, Ayo, ou peſ* |  |
| `c1-l022` | &emsp;*ſoa que tem cuidado dalgum menino nobre.* |  |
| `c1-l023` | Caixei. Mina ſumaſu. *Pagar todo o rendimẽ* |  |
| `c1-l024` | &emsp;*to, ou reſtituir, & pagar todas as diuidas.* |  |
| `c1-l025` | &emsp;*Vt,* Caixeino vqe doriuo toru. *Receber* |  |
| `c1-l026` | &emsp;*conhecimento de como pagou tudo o que deuia.* |  |
| `c1-l027` | Caixet. Firaqi toqu. *Enſinar, & declarar.* |  |
| `c1-l028` | &emsp;*V. G. Lugares eſcuros dalgum liuro, &c.* |  |
| `c1-l029` | Caixiqi. *Raminhos, ou folhas que ſe poem de* |  |
| `c1-l030` | &emsp;*baixo da* Sacana. |  |
| `c1-l031` | Caixǒ. Vmino vye. *Sobre o mar. ¶ Itẽ,* |  |
| `c1-l032` | &emsp;*O mar.* |  |
| `c1-l033` | Caixuno uoxiqi. *Mezas de Iapão vruxadas* |  |
| `c1-l034` | &emsp;*todas de vermelho.* |  |
| `c1-l035` | Caiyeqi. Aratame, cayuru. *Renouar, &* |  |
| `c1-l036` | &emsp;*mudar. (Pirata.* |  |
| `c1-l037` | Caizocu. Vmino nuſubito. *Coſairo, ou* |  |
| `c1-l038` | Caizzuqi. *Cordas com que emlação os remos* |  |
| `c1-l039` | &emsp;*junto das mãos quando ſe rema.* |  |
| `c1-l040` | Caizzuqi. *Balde, ou chareta pera botar fora* |  |
| `c1-l041` | &emsp;*a agoa da bomba.* |  |
| `c1-l042` | Caje. *vento. ¶ Item, Elemento do ar. Vt,* |  |
| `c1-l043` | &emsp;Tçuchi, Mizzu, Caje, Fi. *¶ Itẽ, Ar,* |  |
| `c1-l044` | &emsp;*ou vento frio, ou frialdade. Vt,* Cajeuo fi |  |
| `c1-l045` | &emsp;qu. *Eſtar traſpaſſado do vento, ou doente* |  |
| `c1-l046` | &emsp;*do vento, ou ar. ¶* Chaga caje fiqu. Cu |  |
| `c1-l047` | &emsp;ſurini cajegafiqu, &c. *Danarſe o* Cha, |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO A.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `pn-l001` | 34. |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | &emsp;*ou mezinha, & não ter ſeu vigor, & força cõ* |  |
| `c2-l002` | &emsp;*o ar, ou vento que lhe deu. ¶* Tçuji ca- |  |
| `c2-l003` | &emsp;je. *Redemoinho de vento.* |  |
| `c2-l004` | Cajin. Vtabito. i. Vtayomi. *Poeta.* |  |
| `c2-l005` | Cajiqe, uru, eta. *Empejorar, enfraqueçer, ou* |  |
| `c2-l006` | &emsp;*estar magro, & desfeito. ¶ Item, Permet.* |  |
| `c2-l007` | &emsp;*ſe dira de hum homem pobre, & ſem fato.* |  |
| `c2-l008` | &emsp;*¶* Cajiqebito, l, cajiqeta fito. *Homẽ* |  |
| `c2-l009` | &emsp;*pobre, & neceſſitado.* |  |
| `c2-l010` | Cajit. Natçuno fi. *Dias do verão. P.* |  |
| `c2-l011` | Cajit. Itomanofi. *Diaẽ q̃ hũ fica liure, & deſo* |  |
| `c2-l012` | &emsp;*cupado.* |  |
| `c2-l013` | Cajit. yoi mi. i. yoi conomi. *Boa fruita. P.* |  |
| `c2-l014` | Cama, l, cairu. *Raãs. ¶* Camaquiuin. |  |
| `c2-l015` | &emsp;*Raã, & minhoca.* |  |
| `c2-l016` | Cama. *Caldeirão, ou panela de ferro.* |  |
| `c2-l017` | Cama. *Fouçe de cegar erua.* |  |
| `c2-l018` | Camabiſuxij. *Couſa que faz ruido, ou traqui* |  |
| `c2-l019` | &emsp;*nada, ou fazer traquinada.* |  |
| `c2-l020` | Camaboco. *Hũa certa iguaria de peixe.* |  |
| `c2-l021` | Camachi. *Paos dos cantos, ou bordas de qual* |  |
| `c2-l022` | &emsp;*quer couſa, como armação de portas ainda ſem* |  |
| `c2-l023` | &emsp;*as traueſas, & taboas, &c.* |  |
| `c2-l024` | Camado. *Fogão, ou fornalha onde poem a pa-* |  |
| `c2-l025` | &emsp;*nela de ferro, ou caldeirão.* |  |
| `c2-l026` | Camai, ǒ, ǒta. T*er deuer. ¶* Fitoni |  |
| `c2-l027` | &emsp;camauanu. *Não ter deuer com ninguem.* |  |
| `c2-l028` | Camaite, l, camayete. *Guardai vos, ou o-* |  |
| `c2-l029` | &emsp;*lhai por vos. (onde poem as panelas.* |  |
| `c2-l030` | Camano dan. i. Camado, l, fetçuri. *Lugar* |  |
| `c2-l031` | Camano fa. *Cinta de ferro q̃ tẽ ao derredor das* |  |
| `c2-l032` | &emsp;*bordas algũas panelas de ferro da cozinha.* |  |
| `c2-l033` | Camano feſo. *Remate que fica no fundo da* |  |
| `c2-l034` | &emsp;*panela fundida.* |  |
| `c2-l035` | Camano fitai, l, camadono fitai. *Parte di* |  |
| `c2-l036` | &emsp;*anteira da fornalha mais alta pera que o fogo,* |  |
| `c2-l037` | &emsp;*ou faiſcas não entrem na panela.* |  |
| `c2-l038` | Camano fo. *Eruas compridas que nacem em* |  |
| `c2-l039` | &emsp;*lugares humidos à maneira de candeas amare-* |  |
| `c2-l040` | &emsp;*las. No* Cami *ſe diz* Gamano fo. |  |
| `c2-l041` | Camano maye. *Lar diante do fogão, ou for* |  |
| `c2-l042` | &emsp;*nalha.* |  |
| `c2-l043` | Camano mimi. *Azas da panela, ou buraros* |  |
| `c2-l044` | &emsp;*por onde metem hũas argolas com que a tirão,* |  |
| `c2-l045` | &emsp;*ou poem no fogo.* |  |
| `c2-l046` | Camano xita. *Vão que eſtá dentro do fogão en* |  |
| `c2-l047` | &emsp;*tre olar, & o fundo da panela.* |  |

## Printed signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `sig-l001` | I 2 |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | Cama- |
