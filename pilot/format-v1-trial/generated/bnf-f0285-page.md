# Page view: bnf-f0285

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f285.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **I ANTES DO E.** |  |

## Column 1

Lexical aid: Entry Words Data of Nippojisho, NINJAL, Hideyuki Ohshima and Taichi Aida, version 202510, CC BY 4.0. Source: https://www2.ninjal.ac.jp/textdb_dataset/en/nipp/index.html ; license: https://creativecommons.org/licenses/by/4.0/ . Consulted after independent scan reading; the returned transcription and notes are reviewer-authored, not a reproduction of the index.

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | &emsp;*arſe facilmente à virtude, ou ſer leuado facil-* |  |
| `c1-l002` | &emsp;*mente da virtude.* |  |
| `c1-l003` | Ienyǔ. Yoqi tomo. *Bõs companheiros, ou boa* |  |
| `c1-l004` | &emsp;*companhia.* |  |
| `c1-l005` | Ienzai. Yoqi tacara. *Boas riquezas, ou boa,* |  |
| `c1-l006` | &emsp;*& rica couſa.* |  |
| `c1-l007` | Ienzai. Ycqi cana. *O boa couſa. S.* |  |
| `c1-l008` | Ienzô. i, Ienxǔno ſô. *Religioſo, ou Bon-* |  |
| `c1-l009` | &emsp;*zo da ſeita dos Ienxùs.* |  |
| `c1-l010` | Ieſsǒ. Xitano catachi. *Lingua. S.* |  |
| `c1-l011` | Iet. Xita. *Lingua.* |  |
| `c1-l012` | Ietcot. *Certo lugar dos pees onde ſe dão botões* |  |
| `c1-l013` | &emsp;*de fogo.* |  |
| `c1-l014` | Ietgacu. *Deixar, ou interromper o eſtudo.* |  |
| `c1-l015` | &emsp;*¶ Item, Deixar de eſtudar depois de ſe con-* |  |
| `c1-l016` | &emsp;*ſumar no eſtudo, ou letras.* |  |
| `c1-l017` | Ietguen. Cotobauo tayaſu. i, Gongo dǒdã. |  |
| `c1-l018` | &emsp;*O não poder explicar com palauras. S.* |  |
| `c1-l019` | Ietmet. Taye foroburu. *Deſtruição total. S*. |  |
| `c1-l020` | Ietmiô. *Couſa marauilhoſa, & que não ſe po-* |  |
| `c1-l021` | &emsp;*de explicar com palauras.* |  |
| `c1-l022` | Ietmiǒ. *Principio de deſtruição. Vt,* Co- |  |
| `c1-l023` | &emsp;no vazauaiua cano fitono tameni jetmiǒ- |  |
| `c1-l024` | &emsp;gia. *Eſte trabalho he principio da perdição* |  |
| `c1-l025` | &emsp;*daquelle homem.* |  |
| `c1-l026` | Ietnai. *Pronunciação. Vt,* Ietnaino varui |  |
| `c1-l027` | &emsp;fito. l, yoi fitogia. H*ehomem de mà, ou* |  |
| `c1-l028` | &emsp;*boa pronunciação.* |  |
| `c1-l029` | Iet ri. Taye fanaruru. i, Gongono tayetaru |  |
| `c1-l030` | &emsp;coto. *Falta de palauras, como pera explicar* |  |
| `c1-l031` | &emsp;*algũa couſa.* |  |
| `c1-l032` | Iet rin. i, ſugureta coto. *Couſa excellente.* |  |
| `c1-l033` | Iet riǒ. i, Cocoronimo voyobanu. *Cou-* |  |
| `c1-l034` | &emsp;*ſa que não ſe alcança com o entendimento. Vt,* |  |
| `c1-l035` | &emsp;Ietguen jetrionari. *Não ſe pode explicar* |  |
| `c1-l036` | &emsp;*com palauras, nẽ alcançar com o entendimẽto.* |  |
| `c1-l037` | Iettǒ. Taye tauoruru. i, Cuzzure fateta co |  |
| `c1-l038` | &emsp;to. *Couſa totalmente destruida.* |  |
| `c1-l039` | Iettô. i, Xitano ſaqi. *Ponta da lingua. Vt,* |  |
| `c1-l040` | &emsp;Iettôni noberarenu cotonari. *He couſa q̃* |  |
| `c1-l041` | &emsp;*não ſe pode explicar com palauras. S.* |  |
| `c1-l042` | Iet von. Xita coye. *Voz dos que falão.* |  |
| `c1-l043` | Ietxi, ſuru. Tayuru. l, tatçu. *Cortar, ou* |  |
| `c1-l044` | &emsp;*interromper. Vt,* Xoxinuo jetſuru. *Ha* |  |
| `c1-l045` | &emsp;*muito tempo que não vos viſito por carta.* |  |
| `c1-l046` | &emsp;*¶* Gongouo jetſuru. *Ceßar de falar, ou* |  |
| `c1-l047` | &emsp;*não poder falar, & de clarar algũa couſa.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **I ANTES DO I.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `pn-l001` | **141.** |  |

## Column 2, part 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Iexxe. Yo tayuru. i, Yoni ſugururu coto. |  |
| `c2-l002` | &emsp;*Couſa excellente, ou q̃ excede as idades paſſadas.* |  |
| `c2-l003` | Iexxin. Xita, cuchibiru. *Lingua, & beiços.* |  |
| `c2-l004` | Iexxiqi. *Sentido do gosto.* |  |
| `c2-l005` | Iexxǒ. Tonaye tayuru. *Couſa excellente.* |  |
| `c2-l006` | &emsp;*ou marauilhoſa, & que não ſe pode explicar* |  |
| `c2-l007` | &emsp;*com palauras. Vt,* Qimeô jexxǒno itari |  |
| `c2-l008` | &emsp;nari. *He couſa marauilhoſa. S.* |  |
| `c2-l009` | Iexxǒ. Tamazzuſa tayuru. i, Fumiuo fiſa- |  |
| `c2-l010` | &emsp;xǔ cacanu. *Não eſcreuer carta por muito* |  |
| `c2-l011` | &emsp;*tempo. S.* |  |

## Column 2 internal heading

| Physical line | Main position | Far right |
| --- | --- | --- |
| `s2-01-l001` | **I CONSOANTE AN-** |  |
| `s2-02-l001` | **TES DO I.** |  |

## Column 2, part 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l012` | II. Tera. *Varela, ou moſteiro. Mas naõſe vſa* |  |
| `c2-l013` | &emsp;*ſem algũa compoſiçaõ. Vt* Iichǔ, jinai, |  |
| `c2-l014` | &emsp;jiriǒ. |  |
| `c2-l015` | Ii. Toqi. *Hora: não ſe vſa ſem algũa compo-* |  |
| `c2-l016` | &emsp;*ſição. Vt,* Iiji coccocuni. *Cada hora, ou* |  |
| `c2-l017` | &emsp;*em todo tempo.* |  |
| `c2-l018` | Ii. i, Iitai. *Repugnancia. Vt,* Guioiuo |  |
| `c2-l019` | &emsp;jiſuru. *Repugnar ao que lhe mandão.* |  |
| `c2-l020` | Ii. *Letra. Vt,* Iiuaqega miyenu. *Não* |  |
| `c2-l021` | &emsp;*ſe pode ler eſta letra.* |  |
| `c2-l022` | Ii. Chigo. *Minino. Vſaſe em compoſição de* |  |
| `c2-l023` | &emsp;*outras ſyllabas, ou vocabulos. Vt,* Iixi. |  |
| `c2-l024` | &emsp;*Idem.* Yeiji. i, Midorico. *Idem.* |  |
| `c2-l025` | Ii. Tçuuamono. *Soldados, ou gente de guer-* |  |
| `c2-l026` | &emsp;*ra. Vt,* Yumiǒno jiuo mexiguſu. *Le-* |  |
| `c2-l027` | &emsp;*uar conſigo ſoldados valentes.* |  |
| `c2-l028` | Ii. Coto. *Couſa. Vt,* Ano fito jiga nara- |  |
| `c2-l029` | &emsp;nu. *Aquelle homem he pobre, ou não tem* |  |
| `c2-l030` | &emsp;*poße.* |  |
| `c2-l031` | Iiai. Mizzucara aiſuru. *A mimar, & mo-* |  |
| `c2-l032` | &emsp;*ſtrar amor.* |  |
| `c2-l033` | Iiai. Itçucuximi aiſuru. *A mimar com grã-* |  |
| `c2-l034` | &emsp;*de affeito, & amor.* |  |
| `c2-l035` | Iian. Tera, iuori. *Ermida.* |  |
| `c2-l036` | Iibacu. Mizzucara xibaru. *Atarſe, ou a-* |  |
| `c2-l037` | &emsp;*marrarſe aßi meſmo com algũa ley, ou com a* |  |
| `c2-l038` | &emsp;*mortificação.* |  |
| `c2-l039` | Iibai. Mizzucara vru. *O venderſe a ſi meſ-* |  |
| `c2-l040` | &emsp;*mo. Vt,* Iibai jitocu. *Venderſe aßi meſ-* |  |
| `c2-l041` | &emsp;*mo, & aproueitarſe do preço.* |  |
| `c2-l042` | Iibo. Itçucuximu faua. i, Iifino faua. *Mãy* |  |
| `c2-l043` | &emsp;*piadoſa, ou amoroſa.* |  |
| `c2-l044` | Iibocu. Mimi, me. *Orelhas, & olho. Vt,* |  |

## Printed signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `u2b-l001` | **N n** |  |

## Printed catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `u2b-l001a` |  | Fito |
