# Page view: bnf-f0108

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f108.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO H.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | &emsp;*Riqueza, ou peça rica.* |  |
| `c1-l002` | Chingua. Tacara. *Riquezas. Vt,* Chin- |  |
| `c1-l003` | &emsp;guauo tçumi voqu. *Ter muitas rique-* |  |
| `c1-l004` | &emsp;*zas juntas. S.* |  |
| `c1-l005` | Chingua. Mezzuraxij conomi. *Boa, ou rara* |  |
| `c1-l006` | &emsp;*fruita. Vt,* Chinguauo bucuſuru. *Comer* |  |
| `c1-l007` | &emsp;*fruita noua, ou rara.* |  |
| `c1-l008` | Chinquan. Mezzuraxij motcaſebui. *O eſti-* |  |
| `c1-l009` | &emsp;*mar muito. (mergulharſe.* |  |
| `c1-l010` | Chinrin. i. Xizzumu. *O irſe ao fundo, ou* |  |
| `c1-l011` | Chinſui. Yoi xizzumu. *Bebedice. ¶* Chin- |  |
| `c1-l012` | &emsp;ſui ſuru. *Embebedarſe.* |  |
| `c1-l013` | Chinto. Vt, Chinto xite iru. *Eſtar co-* |  |
| `c1-l014` | &emsp;*mo apartado dos negocios, & trafegos do mun-* |  |
| `c1-l015` | &emsp;*do, ou como ſolitario, & ſoo.* |  |
| `c1-l016` | Chinxeqi. Vare ato. i. Teinchôno ato. *He-* |  |
| `c1-l017` | &emsp;*rança que fica do Rei.* |  |
| `c1-l018` | Chinye. Mezzuraxij coromo. i. Qeccôna |  |
| `c1-l019` | &emsp;ixo. *Veſtido rico, & luſtroſo.* |  |
| `c1-l020` | Chinzai. Tama tacara. *Riquezas. ¶* Chin- |  |
| `c1-l021` | &emsp;zaiuo nague vtẽ chifôſuru. *Fazer aga-* |  |
| `c1-l022` | &emsp;*ſalhido, ou fauorecer alguem não tendo dó das* |  |
| `c1-l023` | &emsp;*riquezas.* |  |
| `c1-l024` | Chiqe. *Certa doença de ſangue.* |  |
| `c1-l025` | Chiqei. Xennogei. i. Anatano yoi gei. |  |
| `c1-l026` | &emsp;*Muitas, & varias viſtas. ¶* Chiqeina |  |
| `c1-l027` | &emsp;tocoro. *Lugar de mil viſtas boas. Per-* |  |
| `c1-l028` | &emsp;*met.* Chiqeino yoifito. *Homem de bõ* |  |
| `c1-l029` | &emsp;*nome, & afamado.* |  |
| `c1-l030` | Chiqemui. *Sangue que ſae fumegando.* |  |
| `c1-l031` | Chiqen. Xirinjiu. *O olhar, ou ver dos* |  |
| `c1-l032` | &emsp;*Fotoqes. Bup.* |  |
| `c1-l033` | Chiqui. *Hum pao do tear em que emburi-* |  |
| `c1-l034` | &emsp;*lhão a ordidura da ſeda.* |  |
| `c1-l035` | Chiracaxi, ſu, aita. Melius chiraxi, ſu. |  |
| `c1-l036` | &emsp;*Eſpalhar, ou derramar.* |  |
| `c1-l037` | Chirachirato. Adu. *Logo depreſſa. ¶ Item,* |  |
| `c1-l038` | &emsp;*Modo de bulir, ou meuer ſe algũa couſa.* |  |
| `c1-l039` | Chiraxi, ſu, aita. *Eſpalhar. ¶* Fanauo |  |
| `c1-l040` | &emsp;chiraſu. *Eſpalhar flores, ou fazer algũa cou-* |  |
| `c1-l041` | &emsp;*ſa limpa, & luzidamente, como que eſpalha* |  |
| `c1-l042` | &emsp;*flores, ou roſas.* |  |
| `c1-l043` | Chiraxi. *Eſpalhamento, ou o eſpalhar.* |  |
| `c1-l044` | Chiren. iqeno ſachiu. *Golfãos, ou ſulas* |  |
| `c1-l045` | &emsp;*deſtes golfãos da lagoa, ou tanque.* |  |
| `c1-l046` | Chiri. *Ciſco.* |  |
| `c1-l047` | Chiri, u, ita. *Eſpalharſe.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO A.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Chiriacu. *Ardil, ou inuenção.* |  |
| `c2-l002` | Chiriana. *Couſa ẽ que deitão o eſterco, ou ciſco.* |  |
| `c2-l003` | Chiribame, uru, eta. *Laurar, ou ornar al-* |  |
| `c2-l004` | &emsp;*gũa couſa cõ ouro, ou prata. ¶* Qinguin- |  |
| `c2-l005` | &emsp;xippôuo chiribamete, rurioi, betta mi- |  |
| `c2-l006` | &emsp;yano vchiye mairi, &c. *Feiq. Lib.* |  |
| `c2-l007` | &emsp;*4. Entrando dentro na ermida do Cami que* |  |
| `c2-l008` | &emsp;*eſtaua toda cozida em ouro, & prata, & or-* |  |
| `c2-l009` | &emsp;*nada de pedras precioſas.* |  |
| `c2-l010` | Chirichirito. Adu. *Modo de correr viua,* |  |
| `c2-l011` | &emsp;*& ligeiramente. ¶ Item, Modo de gote-* |  |
| `c2-l012` | &emsp;*jar, ou cair agoa.* |  |
| `c2-l013` | Chirigami. *Certo papel muito baixo.* |  |
| `c2-l014` | Chirigiri. Adu. *Modo de ſe eſpalhar per* |  |
| `c2-l015` | &emsp;*diuerſas partes. ¶* Chirigirini naru. *Es-* |  |
| `c2-l016` | &emsp;*palharſe, & diuidirſe pera varias partes.* |  |
| `c2-l017` | Chirimidare, ruru, eta. *Eſpalharemſe flores,* |  |
| `c2-l018` | &emsp;*roſas, &c. ſem ordem miſturadas hũas cõ* |  |
| `c2-l019` | &emsp;*outras.* |  |
| `c2-l020` | Chiriqi. Chiyeno chicara. *Força, ou vigor* |  |
| `c2-l021` | &emsp;*de ſaber. ¶* Chiriqiuo tçucuſu. *Eſgotar as* |  |
| `c2-l022` | &emsp;*forças do ſaber, ou meter o reſto do ſaber.* |  |
| `c2-l023` | Chiritcri. *Paa, com que apanhão o ciſco, ou* |  |
| `c2-l024` | &emsp;*couſa ſemelhante.* |  |
| `c2-l025` | Chiriŭ. Todomatte, voſoxi. *Detença, ou* |  |
| `c2-l026` | &emsp;*tardança.* |  |
| `c2-l027` | Chiriuiqi, u, ijta. *Eſtarem folhas, roſas,* |  |
| `c2-l028` | &emsp;*&c. eſpalhadas no chão, ou eſtar o chão co-* |  |
| `c2-l029` | &emsp;*mo juncado, & cuberto deſtas couſas.* |  |
| `c2-l030` | Chirichirito. Adu. *Modo de deſaparecer* |  |
| `c2-l031` | &emsp;*às vezes, & tornar aparecer eſtrela, cometa,* |  |
| `c2-l032` | &emsp;*&c. ¶ Item, Modo de bulir algũa couſa.* |  |
| `c2-l033` | Chiromecaxi, u, aita. *Fazer bulir, ou* |  |
| `c2-l034` | &emsp;*mouer como fogo, ou tição, &c. pera que* |  |
| `c2-l035` | &emsp;*dê lume.* |  |
| `c2-l036` | Chiromeqi, u, eita. *Bulirſe de cá pera lá, ou* |  |
| `c2-l037` | &emsp;*de quando em quando algũa couſa como eſtrela* |  |
| `c2-l038` | &emsp;*por antre nuuẽs que ora ſe vê ora deſaparece.* |  |
| `c2-l039` | Chiſacu. Voſocu tçucuru. *Lauoura, ou* |  |
| `c2-l040` | &emsp;*cultivação feita tarde.* |  |
| `c2-l041` | Chiſan. Voſocu mairu. *Vinda, ou ida tarde.* |  |
| `c2-l042` | Chiſan. Faxe mairu. *Ida, ou vinda apreſſada.* |  |
| `c2-l043` | Chiſato. Xennosato. *Mil lugares, ou mil* |  |
| `c2-l044` | &emsp;*legoas, ou muito longe.* |  |
| `c2-l045` | Chiſô. Vaxiru. *Fauor, & Agazalhado.* |  |
| `c2-l046` | Chiſocu. Voſoicoto, ſunnyacana coto. *Va-* |  |
| `c2-l047` | &emsp;*gar, & preſſa.* |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | Chiji |
