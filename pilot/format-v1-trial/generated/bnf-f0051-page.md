# Page view: bnf-f0051

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f51.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **B ANTES DO A.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Banqe. *Omudarſe, ou trãsformarſe em va-* |  |
| `c1-l002` | &emsp;*rias figuras.* |  |
| `c1-l003` | Banrei. *Todos os viuentes ſenſitiuos.* |  |
| `c1-l004` | Banri. *Dez mil, ou muitas legoas. i. muito* |  |
| `c1-l005` | &emsp;*longe.* |  |
| `c1-l006` | Banri cǒzan. *Hũa pintura antigua do* Cha |  |
| `c1-l007` | &emsp;noyu. |  |
| `c1-l008` | Banrio. Yorozzu vomouacaru. *Diſcurſo,* |  |
| `c1-l009` | &emsp;*ou conſideração de varias couſas.* |  |
| `c1-l010` | Banrǒ. Yorozzuno nami. *Todas as ondas.* |  |
| `c1-l011` | &emsp;*¶ Vt,* Banrǒ banpa. *Idem.* |  |
| `c1-l012` | Banrǒ. Yorozzu itazzugauaxij. i. Yorozzu |  |
| `c1-l013` | &emsp;xinrǒ. *Todos os trabalhos. Vt,* Xencu |  |
| `c1-l014` | &emsp;banrǒuo xinogu. *Paſſar muitos trabalhos.* |  |
| `c1-l015` | Banrui. Yorozzuno tagui. *Todas as eſpe-* |  |
| `c1-l016` | &emsp;*cies, ou layas.* |  |
| `c1-l017` | Banſan. Yorozzuno yama. *Todos os mõtes.* |  |
| `c1-l018` | &emsp;*¶ Vt,* Bãſã Faru itareba mina fanano nixi- |  |
| `c1-l019` | &emsp;qi uo qiru. *Em vindo a primauera todos os* |  |
| `c1-l020` | &emsp;*montes ſe vestem de verduua, & fermoſura.* |  |
| `c1-l021` | Banſô. i. Tomonǒte yuqu ſô. *Religioſo q̃* |  |
| `c1-l022` | &emsp;*vai em companhia de outro.* |  |
| `c1-l023` | Banſot. i. Xoſot. *Todos os ſoldados.* |  |
| `c1-l024` | Banſui. i. Yǔmexi. *Cea, ou comer datarde.* |  |
| `c1-l025` | Bantai. Yorozzu xirizoqu. *Recuſar.* |  |
| `c1-l026` | Bantan. Yorozzuno faxi. *Todas as couſas,* |  |
| `c1-l027` | &emsp;*ou acerca de tudo. ¶ Vt,* Bantan tano- |  |
| `c1-l028` | &emsp;mi zonzuru. *Peço vos que em tudo me fa-* |  |
| `c1-l029` | &emsp;*uoreçais.* |  |
| `c1-l030` | Bante. *Os que vigião reuezando ſe.* |  |
| `c1-l031` | Banten. Cureno ſora. *O ſembrante do tempo* |  |
| `c1-l032` | &emsp;*datarde.* |  |
| `c1-l033` | Banten. Yorozzuno ten. *Todos os ceos.* |  |
| `c1-l034` | Bãtǒ. Yorozzuno cotaye. *Todas as repoſ-* |  |
| `c1-l035` | &emsp;*tas.* |  |
| `c1-l036` | Banxeqi. Curegata. *Sobre atarde.* |  |
| `c1-l037` | Banxi. Yorozzu vomô. *Cuidar muitas* |  |
| `c1-l038` | &emsp;*couſas, ou fazer muitos diſcurſos.* |  |
| `c1-l039` | Banxi. *Vt,* Banxi ixxô. *Dez mil mortes,* |  |
| `c1-l040` | &emsp;*hũa vida, ou das mil partes ſò hũa tem de vida,* |  |
| `c1-l041` | &emsp;*ou eſcaçamente viuera.* |  |
| `c1-l042` | Banxô. Yriaino cane. *Som do ſino que* |  |
| `c1-l043` | &emsp;*tãgem à boca da noite, ou ao pòr do ſol.* |  |
| `c1-l044` | Banxocu. Cureno xocu. *Cea ou comer da* |  |
| `c1-l045` | &emsp;*tarde.* |  |
| `c1-l046` | Banxocu. Cureno iro. i. Bangueino qe |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **B ANTES DO A.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `pn-l001` | 20. |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | &emsp;xiqi. *Sembrante da tarde, ou orizonte da* |  |
| `c2-l002` | &emsp;*tarde.* |  |
| `c2-l003` | Banxu. *Os que vigião.* |  |
| `c2-l004` | Banxû. Aqino cure. i. *Fim do Outono* |  |
| `c2-l005` | &emsp;*que he na nona lũa.* |  |
| `c2-l006` | Banxucqe, l, Fanxucqe. *Hum certo* |  |
| `c2-l007` | &emsp;*paſſaro preto que anda nos rios.* |  |
| `c2-l008` | Banxucqe. *Depois de velho fazerſe religioſo.* |  |
| `c2-l009` | Banxun. Faruno cure. *Fim da primauera* |  |
| `c2-l010` | &emsp;*que he na terceira lũa.* |  |
| `c2-l011` | Banzô. *Terceiro, ou corretor em compras, &* |  |
| `c2-l012` | &emsp;*vendas.* |  |
| `c2-l013` | Banzônin. *Idem.* |  |
| `c2-l014` | Banya. Banno iye. *Caſa de vigia.* |  |
| `c2-l015` | Banyacu. *Officio de vigiar.* |  |
| `c2-l016` | Banyei. Cureno cague. *Sombra da tarde.* |  |
| `c2-l017` | Bappai. *Fazer beber a alguem mais vinho ẽ* |  |
| `c2-l018` | &emsp;*pena de quebrar algũa regra de poliçia açerca* |  |
| `c2-l019` | &emsp;*do beber como ſe faz na China.* |  |
| `c2-l020` | Bappǒ. *Todas as partes.* |  |
| `c2-l021` | Bappon. Motouo nuqu. i. Motouo vchi |  |
| `c2-l022` | &emsp;yaburu. *Deſtruir totalmente algũa couſa de* |  |
| `c2-l023` | &emsp;*raiz.* |  |
| `c2-l024` | Baqe, ru, eta. *Tomar algũa figura, ou trãs-* |  |
| `c2-l025` | &emsp;*formarſe como faz o demonio tomando figura* |  |
| `c2-l026` | &emsp;*de rapoſa, poſto que os Iapões não nomeão o dia* |  |
| `c2-l027` | &emsp;*bo, mas dizem Qitçunega baqeta. A-* |  |
| `c2-l028` | &emsp;*rapoſa ſe fez homem, ou apareçeo trãsformada* |  |
| `c2-l029` | &emsp;*ẽ molher, homẽ, &c. ¶ Item, permet.* |  |
| `c2-l030` | &emsp;*Mudar otrajo, & tomar figura eſtranha. Vt* |  |
| `c2-l031` | &emsp;Vonnani bacuru. *Veſtirſe o homem ẽ trajos* |  |
| `c2-l032` | &emsp;*de molher.* |  |
| `c2-l033` | Baqe. *Figura que ſe transformou noutra, como* |  |
| `c2-l034` | &emsp;*odemonio em rapoſa, ou a rapoſa, como cuidão* |  |
| `c2-l035` | &emsp;*os gentios, ou adibe em homem, molher, &c.* |  |
| `c2-l036` | &emsp;*¶* Baqega miyeta, l, arauareta. *Deſcobrio* |  |
| `c2-l037` | &emsp;*ſe ja que era rapoſa, ou adibe, &c tranſ-* |  |
| `c2-l038` | &emsp;*formado noutra figura. ¶ Item, Permet.* |  |
| `c2-l039` | &emsp;*Engano, ou diſſimulação com que hum engana a* |  |
| `c2-l040` | &emsp;*outro como o que ſe faz uirtuoſo, ou religioſo* |  |
| `c2-l041` | &emsp;*ſendo mao, & ſecular, &c. ¶* Baqega |  |
| `c2-l042` | &emsp;arauareta. *Deſcobrioſe o engano, ou diſſimu* |  |
| `c2-l043` | &emsp;*lação, &c.* |  |
| `c2-l044` | Baqebaqexij. i. Baqeraxij. *Couſa que pa-* |  |
| `c2-l045` | &emsp;*rece como transformada, & contra feita, ou* |  |
| `c2-l046` | &emsp;*extra ordinaria no trajo, & modo, como mo-* |  |

## Signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `sig-l001` | E 4 |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | lher |
