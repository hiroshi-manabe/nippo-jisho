# Page view: bnf-f0116

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f116.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO O.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | &emsp;*entre duas faxiras pera as ter dereitas, & aju-* |  |
| `c1-l002` | &emsp;*dar a ſoſtentar.* |  |
| `c1-l003` | Cobaxi, ſu, aita. *Fazer que a couſa ſeja* co |  |
| `c1-l004` | &emsp;bita*, ou tenha algũa particularidade, ou graça* |  |
| `c1-l005` | &emsp;*differente das outras. ¶* Cobaite yù. *Fa-* |  |
| `c1-l006` | &emsp;*lar palauras ſelectas, & que os outros ordina-* |  |
| `c1-l007` | &emsp;*riamente não falão.* |  |
| `c1-l008` | Cǒbaxij. *Couſa cheiroſa, ou cheirar bem.* |  |
| `c1-l009` | &emsp;¶ Cǒbaxiſa. |  |
| `c1-l010` | &emsp;Cǒbaxù. |  |
| `c1-l011` | Cobayai. *Molher que pare, ou emprenha mui-* |  |
| `c1-l012` | &emsp;*tas vezes, ou facilmente.* |  |
| `c1-l013` | Cǒbe. *Cabeça. ¶* Cǒbeuo vnataruru. |  |
| `c1-l014` | &emsp;*Abaixar, ou inclinar a cabeça pera diante co-* |  |
| `c1-l015` | &emsp;*mo quem faz reuerencia, &c.* |  |
| `c1-l016` | Cobi. Torano vo. *Rabo de tigre.* |  |
| `c1-l017` | Cobi. i. Itçucuxij. *Couſa fermoſa, & gracio-* |  |
| `c1-l018` | &emsp;*ſa. ¶* Fitotabi yemeba momono cobi |  |
| `c1-l019` | &emsp;nari. *Rindoſe hũa vez reſultauão mil gra* |  |
| `c1-l020` | &emsp;*ças no roflo.* |  |
| `c1-l021` | Cobi fetçurai, ǒ. *Lizonjear, ou falar cõ* |  |
| `c1-l022` | &emsp;*certa graça pera contentar.* |  |
| `c1-l023` | Cobi, uru, ita. *Ter a couſa em ſi algũa gra-* |  |
| `c1-l024` | &emsp;*ça, ou particularidade que contenta. ¶ Itẽ,* |  |
| `c1-l025` | &emsp;*Ser* Cobito, *& eſtrauagante em falar pala-* |  |
| `c1-l026` | &emsp;*uras nouas, & exquiſitas. Vt,* Cobite yǔ. |  |
| `c1-l027` | &emsp;*Falar eſtas palauras extraordinarias, & ſe-* |  |
| `c1-l028` | &emsp;*lectas.* |  |
| `c1-l029` | Cobin. *Cabellos das fontes, ou as meſmas fon* |  |
| `c1-l030` | &emsp;*tes. ¶* Cobinuo caqu. *Coçar as fontes* |  |
| `c1-l031` | &emsp;*com pezar, ou deſgosto dalgũa couſa.* |  |
| `c1-l032` | Cǒbin. Saiuaino tayori. *Bom portador, ou* |  |
| `c1-l033` | &emsp;*oportunidade, & conjunção. S.* |  |
| `c1-l034` | Cǒbin. Conomaxij, l, yoi tayori. *Mẽ-* |  |
| `c1-l035` | &emsp;*ſajeiro, ou portador, ou oportunidade.* |  |
| `c1-l036` | Cobǒ. *Moçozinho rapado que ſerue como* |  |
| `c1-l037` | &emsp;Comono*.* |  |
| `c1-l038` | Cobochi, tçu, otta. *Desfazer. ¶* Iye |  |
| `c1-l039` | &emsp;uo cobutçu. *Desfazer, ou deſmanchar* |  |
| `c1-l040` | &emsp;*a caſa.* |  |
| `c1-l041` | Côbocu. *Certa mezinha.* |  |
| `c1-l042` | Cobocu. Care qi. *Aruore ja ſeca.* |  |
| `c1-l043` | Cobocu. *Arvore velha.* |  |
| `c1-l044` | Cobore, uru, eta. *Entornarſe, ou tresbordar.* |  |
| `c1-l045` | &emsp;¶ Namidaga coboruru. *Cairem as lagri* |  |
| `c1-l046` | &emsp;*mas dos olhos. ¶* Catanano faga cobo- |  |
| `c1-l047` | &emsp;reta. *Fazeremſe moças no corte da* Catana. |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO H.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Cobore cacari, u, atta. *Estar a couſa tras* |  |
| `c2-l002` | &emsp;*bordando, & caindo ſobre outra.* |  |
| `c2-l003` | Coboreuochi, uru, ita. *Trasbordar, ou cair.* |  |
| `c2-l004` | Coboxi, ſu, oita. *Entornar, ou derramar.* |  |
| `c2-l005` | &emsp;¶ Namidauo coboſu. *Derramar lagri-* |  |
| `c2-l006` | &emsp;*mas. ¶* Catanano fauo coboſu. *Fa-* |  |
| `c2-l007` | &emsp;*zer moças no corte da* catana. |  |
| `c2-l008` | Cobǒxi. *Bonzinho, ou Menino rapado.* |  |
| `c2-l009` | Cobu. *Lobinho, ou carne ſaida pera fora* |  |
| `c2-l010` | &emsp;*naturalmente.* |  |
| `c2-l011` | Cobu. *Hũs limos domar de folha larga que* |  |
| `c2-l012` | &emsp;*ſe comem.* |  |
| `c2-l013` | Cobucu. *Hũa vez pequena de* Cha, *ou de* |  |
| `c2-l014` | &emsp;*mezinha que ſe bebe.* |  |
| `c2-l015` | Cobucuro. *Bolſinha, ou ſaquinho.* |  |
| `c2-l016` | Cobujiqirô. *Certa boceta redonda.* |  |
| `c2-l017` | Cobun. Furui fumi. *Eſcritura antigua.* |  |
| `c2-l018` | &emsp;¶ *Item nome de hum certo liuro de bom es* |  |
| `c2-l019` | &emsp;*tillo pera compor.* |  |
| `c2-l020` | Cǒbun. Tçuguru fumi. *Carta, ou papel* |  |
| `c2-l021` | &emsp;*em que ſe notifica algũa couſa certa a alguem.* |  |
| `c2-l022` | Cǒbun. *Eſcritura de eſtilo alto, & excellẽte.* |  |
| `c2-l023` | Cobune. *Embarcação pequena.* |  |
| `c2-l024` | Cobura. *Barriga da perna.* |  |
| `c2-l025` | Cobut. Inixiyeno fotoqe. Fotoqe *antiguo.* |  |
| `c2-l026` | Cǒbut. Yoximono, l, conomu mono. *Cou-* |  |
| `c2-l027` | &emsp;*ſa boa, ou apeticiuel como couſa que o do-* |  |
| `c2-l028` | &emsp;*ente apetece, ou que hum folga, & gosta* |  |
| `c2-l029` | &emsp;*de comer, &c. ¶ Item couſa que he boa* |  |
| `c2-l030` | &emsp;*pera a ſaude, & não preiudica às mezinhas* |  |
| `c2-l031` | &emsp;*que hum toma, &c.* |  |
| `c2-l032` | Cobuxi. *Punho cerrado. ¶* Cobuxide |  |
| `c2-l033` | &emsp;yamaſuru. *Dar de punhadas.* |  |
| `c2-l034` | Cobuxi. *Certa raiz de erua medicinal.* |  |
| `c2-l035` | Cǒca. Tacai norimono. *Andas de peſſoa* |  |
| `c2-l036` | &emsp;*nobre.* |  |
| `c2-l037` | Côca. Xiriyeno tana. i. Xǒbenjo. *Secre* |  |
| `c2-l038` | &emsp;*tas de ourinar. Palaura de molheres.* |  |
| `c2-l039` | Coca. Furui vta. *Verſos antigos.* |  |
| `c2-l040` | Cǒcacu. Yuqu marebito. *Peregrino.* |  |
| `c2-l041` | Cocacu. Qitçune, mujina. *Rapoſa, &* |  |
| `c2-l042` | &emsp;*outro animal ſemelhante a adibe. ¶* Coca |  |
| `c2-l043` | &emsp;cunocauagoromoua carù xite fanafada cã |  |
| `c2-l044` | &emsp;uo fuxegu. *O veſtido feito das pelles deſtes* |  |
| `c2-l045` | &emsp;*animaes ſendo leue defende grãdemente do frio.* |  |
| `c2-l046` | Cǒcacu. Tacai iye. *Caſa alta, ou edificio no* |  |
| `c2-l047` | &emsp;*bre, & alto.* |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `cw-l001` | **Coca-** |  |
