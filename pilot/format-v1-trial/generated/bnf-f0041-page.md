# Page view: bnf-f0041

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f41.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **A ANTES DO T.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | &emsp;*do, & aſolado depois de ſe ir algũa gente co-* |  |
| `c1-l002` | &emsp;*mo ſoldados que fazẽ inſultos, &c. ¶* Ta- |  |
| `c1-l003` | &emsp;tçu torimo atouo nigoſanu. *Prou. Quã-* |  |
| `c1-l004` | &emsp;*do ſe hum aparta de hum lugar, deixalo bem* |  |
| `c1-l005` | &emsp;*concertado, como paſſaro que aleuãtandoſe da* |  |
| `c1-l006` | &emsp;*agoa a deixa limpa. ¶* Atouo tçucuru. |  |
| `c1-l007` | &emsp;*Deixar raſto de ſi. ¶* Atouo tçuguru, l, |  |
| `c1-l008` | &emsp;atouo tçuguete yuqu. *Ir de traz no alcãçe* |  |
| `c1-l009` | &emsp;*dos que vão fugindo, ou dalgum animal, ou* |  |
| `c1-l010` | &emsp;*aue. ¶* Atouo ſaqini, l, ſaqiye ſuru, |  |
| `c1-l011` | &emsp;l, naſu. *Fazer depois o q̃ auia de fazer pri-* |  |
| `c1-l012` | &emsp;*meiro. ¶* Atouo curamacaſu. *Quando* |  |
| `c1-l013` | &emsp;*hum foze fazer que não ſe ſaiba nẽ fique ra-* |  |
| `c1-l014` | &emsp;*ſto, ou ſinal. ¶ Itẽ,* Ato. *Significa pès.* |  |
| `c1-l015` | &emsp;*Vt,* Atouo tçugǒ, l, atouo ſaſu. *Dormir hũ* |  |
| `c1-l016` | &emsp;*com os pès pera outro, ou com os pès trauados* |  |
| `c1-l017` | &emsp;*hũ cõ o outro. ¶* Atouo taruru. *Manifeſtar* |  |
| `c1-l018` | &emsp;*ſe neſte mundo o Cami, ou Fotoqe tomando* |  |
| `c1-l019` | &emsp;*corpo, & deixando exemplo. No Buppô ſe diz* |  |
| `c1-l020` | &emsp;ſuixacu. *Podeſe acomodar à encarnação do filho* |  |
| `c1-l021` | &emsp;*de Deos, & ao exemplo que deixou neſte mundo* |  |
| `c1-l022` | &emsp;*cõ ſua vida, & Doutrina. ¶ Item,* Ato. *Co-* |  |
| `c1-l023` | &emsp;*ſtas, ou de traz. Vt,* Atoni mairu. *Ir de traz,* |  |
| `c1-l024` | &emsp;*ou nas coſtas dalguem.* |  |
| `c1-l025` | Atocata. *Vt,* Atocatamo nai coto. *Couſa de* |  |
| `c1-l026` | &emsp;*que ſe não ſabe parte nẽ raſto. i. não tem* |  |
| `c1-l027` | &emsp;*nenhũa noticia.* |  |
| `c1-l028` | Atome. *Socceſſor, ou herdeiro. ¶ Item, He-* |  |
| `c1-l029` | &emsp;*rança. ¶* Atomeuo vqetoru. *Tomar* |  |
| `c1-l030` | &emsp;*entrega da herança.* |  |
| `c1-l031` | Atoniguiuai. *Vide ſupra,* Ato. |  |
| `c1-l032` | Atoſaqi. i. Iengo. *De traz, & de diante, ou ãte-* |  |
| `c1-l033` | &emsp;*cedente, & conſequente. ¶* Ato ſaqicara |  |
| `c1-l034` | &emsp;qicoyuru. *Entenderſe do que fica atraz, &* |  |
| `c1-l035` | &emsp;*vai a diante.* |  |
| `c1-l036` | Atotçugui. *Herdeiro, ou ſocceſſor.* |  |
| `c1-l037` | Atoxiqi. *Bens, ou fazenda que fica em herãça.* |  |
| `c1-l038` | Atoya macura. *Pès, & cabeçeira. ¶ Vt,* |  |
| `c1-l039` | &emsp;Atoya macurani tachiyorite. *Chegandoſe* |  |
| `c1-l040` | &emsp;*aos pès, ou a cabeçeira.* |  |
| `c1-l041` | Atqet. *Vide* Atqet. |  |
| `c1-l042` | Atqi. *Vide* Acqi. |  |
| `c1-l043` | Attori. *Hũ certo paſſarinho. Vt,* Attorino |  |
| `c1-l044` | &emsp;fini vochica yǒni futamequ. *Prou. Pollo* |  |
| `c1-l045` | &emsp;*ſobreſalto, & toruação não ſe ſaber dar a cõ-* |  |
| `c1-l046` | &emsp;*ſelho perturbandoſe como eſte paſſarinho quãdo* |  |
| `c1-l047` | &emsp;*cae no fogo.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **A ANTES DO V.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `pn-l001` | 15. |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | AVA. *Painço, ou milho, hũa das cinco* |  |
| `c2-l002` | &emsp;*ſementes, ou legumes chamados Gococu.* |  |
| `c2-l003` | Aua. *Eſcuma.* Auauo fuqu. *Eſcumar polla boca.* |  |
| `c2-l004` | &emsp;*¶* Auaga tatçu. *Leuantarſe eſcuma.* |  |
| `c2-l005` | &emsp;*¶* Auauo toru. *Tirar a eſcuma da panella.* |  |
| `c2-l006` | Auabi, l, vǒbi. *Lapas. ¶* Cuxi auabi. |  |
| `c2-l007` | &emsp;*Lapas ſecas eſpetadas. ¶* Xiuo auabi. *La-* |  |
| `c2-l008` | &emsp;*pas ſalgadas.* |  |
| `c2-l009` | Auagai. *Papas, ou canja de painço.* |  |
| `c2-l010` | Auaiy. i. Auainexi. *Comer feito de painço à* |  |
| `c2-l011` | &emsp;*maneira de arroz cozido.* |  |
| `c2-l012` | Auai. *Couſa enxabida como comer enxabido,* |  |
| `c2-l013` | &emsp;*& ſem ſabor. Inuſit.* |  |
| `c2-l014` | Auai. *Eſpaço de lugar, ou tempo. ¶ Item,* |  |
| `c2-l015` | &emsp;*Entre, ou no meo. Vt,* Auaini iru. *Eſtar no* |  |
| `c2-l016` | &emsp;*meo, ou antre outras couſas.* |  |
| `c2-l017` | Auamochi. *Bolos feitos de painço, ou milho.* |  |
| `c2-l018` | Auare. *Praza a Deos, oxalà. Segueſe lhe ſempre* |  |
| `c2-l019` | &emsp;Caxi. *Vt,* Auare coreuo xeyocaxi. *Oxalà* |  |
| `c2-l020` | &emsp;*que façais iſto.* |  |
| `c2-l021` | Auare. *Miſericordia, ou cõpaixão. ¶* Auare |  |
| `c2-l022` | &emsp;uo moyouoſu. *Mouerſe a cõpaixão. ¶* Auare- |  |
| `c2-l023` | &emsp;ni vomǒ. *Ter compaixão, piedade.* |  |
| `c2-l024` | Auaremi. *Piedade, Miſericordia. ¶* Auaremi |  |
| `c2-l025` | &emsp;uo cuuayuru, l, taruru. *Auer miſericor-* |  |
| `c2-l026` | &emsp;*dia, ou, compadeçerſe. ¶* Auaremiuo cõmu- |  |
| `c2-l027` | &emsp;ru. *Receber miſericordia, ou boa obra de* |  |
| `c2-l028` | &emsp;*outro.* |  |
| `c2-l029` | Auaremi, u. *Ter miſericordia, ou apie-* |  |
| `c2-l030` | &emsp;*darſe.* |  |
| `c2-l031` | Auarena. *Couſa piadoſa, ou que cauſa laſtima,* |  |
| `c2-l032` | &emsp;*& compaixão.* |  |
| `c2-l033` | &emsp;Auareni. |  |
| `c2-l034` | &emsp;Auareſa. |  |
| `c2-l035` | Auatataxij. *Açodado, leue, ou perturbado* |  |
| `c2-l036` | &emsp;*em fazer algũa couſa.* |  |
| `c2-l037` | &emsp;Auatataxiſa. |  |
| `c2-l038` | &emsp;Auatataxǔ. |  |
| `c2-l039` | Auate, tçuru, teta. *Perturbarſe com grande* |  |
| `c2-l040` | &emsp;*preſſa, ou eſtar perturbado. ¶* Monouo |  |
| `c2-l041` | &emsp;auateteſuru. *Fazer algũa couſa apreſſada,* |  |
| `c2-l042` | &emsp;*ou perturbadamente.* |  |
| `c2-l043` | Auate fatameqi, u, eita. *Perturbarſe açoda-* |  |
| `c2-l044` | &emsp;*damente.* |  |
| `c2-l045` | Auateguini. *Adu. i.* Auatataxǔ qiruuci. *Cõ* |  |
| `c2-l046` | &emsp;*perturbação, & preſſa no veſtir, armarſe, &c.* |  |
| `c2-l047` | Auateſauagui, u, aida. *Apreſſarſe com pertur-* |  |

## Signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `sig-l001` | D 3 |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | buição |
