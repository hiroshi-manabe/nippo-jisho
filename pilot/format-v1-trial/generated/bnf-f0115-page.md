# Page view: bnf-f0115

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f115.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO H.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Chùv. Bup. Nacani ari. *estar no meo. i.* |  |
| `c1-l002` | &emsp;*ſem ir a pena, nem a gloria.* |  |
| `c1-l003` | Chûuǒ. Naca, naca. *Meo. ¶* Ten no |  |
| `c1-l004` | &emsp;chùuǒ. *Meo doceo.* |  |
| `c1-l005` | Chùuǒ. *Hum certo banquinho ſobre q̃ poem o* |  |
| `c1-l006` | &emsp;*perfumador.* |  |
| `c1-l007` | Chùxet. *Seruiçoinſigne.* |  |
| `c1-l008` | Chùxi cumono ito. *Fios da tea da aranha.* |  |
| `c1-l009` | &emsp;*vt,* chùxi qireyaſuxi. *Os fios da tea* |  |
| `c1-l010` | &emsp;*daranha quebram ſe facilmente.* |  |
| `c1-l011` | Chùxin. *Dar nouas ao ſenhor do arrayal, do ſuc-* |  |
| `c1-l012` | &emsp;*ceſſo da gerra, ou de outra couſa. ¶* Chùxin |  |
| `c1-l013` | &emsp;uo mǒſu, l, ſuru. *Dar eſtas nouas, ou fazè* |  |
| `c1-l014` | &emsp;*las a ſaber.* |  |
| `c1-l015` | Chùxin. Tadaxijxinca. *Criado de grandes ſer-* |  |
| `c1-l016` | &emsp;*uiços.* |  |
| `c1-l017` | Chùxǒ. *Premio dos ſeruiços.* |  |
| `c1-l018` | Chùxocun. *Pena deſcreuer.* |  |
| `c1-l019` | Chûxu. *Vinho queſe dà na meza.* |  |
| `c1-l020` | Chùxù. l, chùjù. *Meo do outono, ou,* |  |
| `c1-l021` | &emsp;*oitaua lũa. vt,* chùxù tenqi fogaraca |  |
| `c1-l022` | &emsp;nari. *A lùa do meo do outono he muito clara.* |  |
| `c1-l023` | Chùxun, l, Chùjun. Niguatno tçuqino coto. |  |
| `c1-l024` | &emsp;*Segundo mez, ou lũa, ou meo da prima* |  |
| `c1-l025` | &emsp;*Vera.* |  |
| `c1-l026` | Chùya. firu, yoru. *De dia, & de noite.* |  |
| `c1-l027` | Chùyǒ. *Segunda lũa, ou mez.* |  |
| `c1-l028` | Chùyô. *Meyo, ou mediocridade nas couſas.* |  |
| `c1-l029` | &emsp;*Vt,* Banji chùyôuo mamore. *Em tudo* |  |
| `c1-l030` | &emsp;*guardar o meo, ou em tudo ſede temperado.* |  |
| `c1-l031` | &emsp;¶ I*tem, Nome de hum liuro da china que* |  |
| `c1-l032` | &emsp;*trata deſta temperança, & mediocridade.* |  |
| `c1-l033` | Chùzon. Fotoqe *que esta no meyo, i. en-* |  |
| `c1-l034` | &emsp;*tre dous.* |  |

## Printed alphabetical divider

| Physical line | Main position | Far right |
| --- | --- | --- |
| `d1-l001` | **C ANTES DO O.** |  |

## Column 1 continuation

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l035` | Cǒ. *Obediencia de filho pera opay, ou mãy.* |  |
| `c1-l036` | &emsp;*Vt,* Qimini chùfucacu, bumoni cǒ |  |
| `c1-l037` | &emsp;aru fito. *Homem de ſeruiços pera com o* |  |
| `c1-l038` | &emsp;*ſenhor, & obediente aos pais.* |  |
| `c1-l039` | Cô. *Hum certo paſſaro grande.* |  |
| `c1-l040` | Cô. *Exercicio, ou habito. Vt,* Côga itta. |  |
| `c1-l041` | &emsp;*Eſtar habituado, ou exercitado. ¶* Ien, l, |  |
| `c1-l042` | &emsp;gacumonno cô uo tçumu. *Ter acquiri* |  |
| `c1-l043` | &emsp;*do grande habito de virtude, ou das letras.* |  |
| `c1-l044` | &emsp;¶ *item, cõ ſeruiços grandes que* |  |
| `c1-l045` | &emsp;*ſe fazem a algum ſenhor. ¶* Cô aru fito. |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO O.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `p-l001` | **52.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | &emsp;*Homem de grandes ſeruiços.* |  |
| `c2-l002` | Co. *Filho. ¶* Couo mǒquru. *Auer fi* |  |
| `c2-l003` | &emsp;*lho. ¶* Couo nagaſu. *Botar a criança* |  |
| `c2-l004` | &emsp;*que não esta ainda bem coalhada no ventre.* |  |
| `c2-l005` | &emsp;¶ Couo daqu. *Trazer o menino no collo.* |  |
| `c2-l006` | Cǒ. *Aguila. ¶* Cǒuo taqu. *Queimar* |  |
| `c2-l007` | &emsp;*aguila pera cheirar, ou perfumar. ¶* Cǒ |  |
| `c2-l008` | &emsp;uo tomuru. *Perfumarſe, ou aos veſtidos* |  |
| `c2-l009` | &emsp;*com aguila. ¶* Cǒuo qiqu. *Cheirar al* |  |
| `c2-l010` | &emsp;*gũa aguila, ou calamſa no perfumador, &* |  |
| `c2-l011` | &emsp;*ver, ou julgar a qualidade de cheiro.* |  |
| `c2-l012` | Cǒacu. Yoxi, axi. *Couſa boa, ou apeticiuel,* |  |
| `c2-l013` | &emsp;*& ruim, ou que não he apeticiuel.* |  |
| `c2-l014` | Coai. *Trutas pequenas.* |  |
| `c2-l015` | Côan. Nochiuo cangǒru. *Conſideração do* |  |
| `c2-l016` | &emsp;*futuro.* |  |
| `c2-l017` | Coaxi. *Esteyo.* |  |
| `c2-l018` | Coaxi. *Paſſo pequeno. ¶* Coaxini ayu- |  |
| `c2-l019` | &emsp;mu. *Andar com o paſſo curto.* |  |
| `c2-l020` | Coba. i. Cococuno vma, l, yebiſuno v- |  |
| `c2-l021` | &emsp;ma. *Caualo de hum Reino vezinho ao da* |  |
| `c2-l022` | &emsp;*China chamado.* Coba. |  |
| `c2-l023` | Cǒba. Yuqu vma. *Caualo que caminha.* |  |
| `c2-l024` | Cǒbaco. *Bocetinha de cheiros.* |  |
| `c2-l025` | Cobaco. *Bocetinha em que metem algũa me-* |  |
| `c2-l026` | &emsp;*zinha, &c.* |  |
| `c2-l027` | Côbai. *Queda que ſe deixa nos telhados pera* |  |
| `c2-l028` | &emsp;*correr a agoa da chuua. ¶* Côbaiga fayai, |  |
| `c2-l029` | &emsp;l, nurui. *Ser aqueda do telhado muito in-* |  |
| `c2-l030` | &emsp;*greme, ou muito deſcanſada, & pouco ingre-* |  |
| `c2-l031` | &emsp;*me.* |  |
| `c2-l032` | Cobami, u. i. Tatezzuqu. *Reſistir, ou* |  |
| `c2-l033` | &emsp;*moſtrar repugnancia ao que ſe manda, ou a* |  |
| `c2-l034` | &emsp;*conſelha.* |  |
| `c2-l035` | Coban. *Taboa que poem no fundo das gayolas* |  |
| `c2-l036` | &emsp;*dos paſſaros.* |  |
| `c2-l037` | Coban. Fiuo tçugǒ Ienqeno mexiuo ſuru fi |  |
| `c2-l038` | &emsp;taqi. *Homem de ſeruiço que nas Teras dos* |  |
| `c2-l039` | &emsp;*Ienxùs ſerue de cozinheiro.* |  |
| `c2-l040` | Cobari. *Traue pequena, ou tirante que atra* |  |
| `c2-l041` | &emsp;*ueſa no edificio, ou madeiramento.* |  |
| `c2-l042` | Cǒbari. *Pao, ou bambu, &c. que ſe mete an* |  |
| `c2-l043` | &emsp;*tre algũa couſa que ſe poema enxugar pera q̃* |  |
| `c2-l044` | &emsp;*ſe não pegue hũa parte com outra. ¶* Coba |  |
| `c2-l045` | &emsp;riuo cǒ. *Meter este pao, ou couſa ſeme* |  |
| `c2-l046` | &emsp;*lhante pera eſte effeito. ¶ Item, Por algũ* |  |
| `c2-l047` | &emsp;*pao como de emprestado, & por pouco tempo-* |  |

## Printed signature mark

| Physical line | Main position | Far right |
| --- | --- | --- |
| `s-l001` | **N 4** |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `cw-l001` | **ent-** |  |
