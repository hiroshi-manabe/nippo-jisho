# Page view: bnf-f0089

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f89.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **C ANTES DO A.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | &emsp;Sura caqicumoru. |  |
| `c1-l002` | Caqicuraxi, u, aita. *Eſcureçer. Vt,* Yono- |  |
| `c1-l003` | &emsp;naca caqicuraſu cocochi. *O mundo feito* |  |
| `c1-l004` | &emsp;*como às eſcuras. i. O imperio, ou reino per-* |  |
| `c1-l005` | &emsp;*turbado, &c.* |  |
| `c1-l006` | Caqicure, uru, eta. *Eſcurecerſe, ou apagar-* |  |
| `c1-l007` | &emsp;*ſe. Vt,* Fumiuo aqete mirarureba, fu- |  |
| `c1-l008` | &emsp;deno atoua namidanicaqicurete, ſoco fa- |  |
| `c1-l009` | &emsp;catouamiyenu. Feiq*. Lib. 1. Abrĩ* |  |
| `c1-l010` | &emsp;*do, & vendo a carta eſtauão as letras com as* |  |
| `c1-l011` | &emsp;*lagrimas apagadas, de maneira que não ſe diſ-* |  |
| `c1-l012` | &emsp;*tinguião bem.* |  |
| `c1-l013` | Caqicuuaye, uru, eta. *Eſcreuer ajuntando* |  |
| `c1-l014` | &emsp;*ao que eſtà eſcrito. ¶ Item, Permet.* |  |
| `c1-l015` | &emsp;*Contar a algum, ou metelo no numero de outros.* |  |
| `c1-l016` | Caqicuyǒji, uru, ita. *Eſcreuendo algũa cou-* |  |
| `c1-l017` | &emsp;*ſa offerecela ao* fotoqe. |  |
| `c1-l018` | Caqidaxi, ſu, aita. T*irar algũa couſa aos hõ-* |  |
| `c1-l019` | &emsp;*bros, ou nas mãos. Vt,* Ano biǒbuuo |  |
| `c1-l020` | &emsp;caqidaxe. T*irai fora aquelle* biǒbu. |  |
| `c1-l021` | Caqidouori. *Erua aſſi chamada. No* cami |  |
| `c1-l022` | &emsp;*ſe diz* Caqidouoxi. |  |
| `c1-l023` | Caqifuxe, uru, eta. T*omando a alguem* |  |
| `c1-l024` | &emsp;*nas mãos deitalo. Vt,* Fizano vyeni ca- |  |
| `c1-l025` | &emsp;qi fuxeta. *Encoſtouo ſobre os joelhos, ou* |  |
| `c1-l026` | &emsp;*pollo no regaço.* |  |
| `c1-l027` | Caqi idaqi, u, aita. T*omar nos braços.* |  |
| `c1-l028` | Caqi idaxi, ſu, aita. *vide* Caqidaſu. |  |
| `c1-l029` | Caqi idaxi, ſu, aita. T*irar a lume, ou ſair* |  |
| `c1-l030` | &emsp;*com algũa boa forma de letra de Iapão.* |  |
| `c1-l031` | Caqimaje, zuru, jeta. *Mexer algũa couſa.* |  |
| `c1-l032` | Caqimauaxi, ſu, aita. *Fazer algũa roda,* |  |
| `c1-l033` | &emsp;*ou circulo.* |  |
| `c1-l034` | Caqimidare, ruru, eta. *Perturbarſe. ¶* Co- |  |
| `c1-l035` | &emsp;coro caqimidarete. *Perturbando ſe o co-* |  |
| `c1-l036` | &emsp;*ração.* |  |
| `c1-l037` | Caqimidaxi, ſu, aita. *Emburilhar, ou des* |  |
| `c1-l038` | &emsp;*concertar. Vt,* Cami caqimidaſu. *Des* |  |
| `c1-l039` | &emsp;*concertar, & empeçar os cabellos.* |  |
| `c1-l040` | Caqin. Tamano qizu. i. Chijocu. *Inju-* |  |
| `c1-l041` | &emsp;*ria. Vt,* Xǒgaino caqin nari. *He a* |  |
| `c1-l042` | &emsp;*fronta em quanto viuer, ou de toda a vida.* |  |
| `c1-l043` | Caqinade, zzuru, deta. *Affàgar pondo a* |  |
| `c1-l044` | &emsp;*mão na cabeça, ou outra parte. Vt,* Vo- |  |
| `c1-l045` | &emsp;cureno camiuo caqinadeta. *Poz lhe a* |  |
| `c1-l046` | &emsp;*mão nos cabellos q̃ caẽ das fontes pera baxo.* |  |
| `c1-l047` | Caqinarabe, uru, eta. *Leuarem muitos al-* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **C ANTES DO A.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `pn-l001` | 39. |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | &emsp;*gũa couſa nas mãos, ou aos hombros igualmẽte.* |  |
| `c2-l002` | Caqinaraxi, ſu, aita. *Aprainar a terra.* |  |
| `c2-l003` | &emsp;*Vt,* Giuo caqinaraſu. |  |
| `c2-l004` | Caqinauoxi, ſu, oita. *Tornar a eſcreuer mi* |  |
| `c2-l005` | &emsp;*lhor o mal eſcrito.* |  |
| `c2-l006` | Caqinaxi, ſu, aita. *Eſcreuer de maneira que* |  |
| `c2-l007` | &emsp;*pareça aſſi, ou aſſi. Vt,* Itçuuarino macoto |  |
| `c2-l008` | &emsp;no yǒni caqinaxite. *Eſc*r*endo a mintira* |  |
| `c2-l009` | &emsp;*de maneira que parecia verdade.* |  |
| `c2-l010` | Caqine. *Raiz, ou pè da ſebe.* |  |
| `c2-l011` | Caqinobori, ru, otta. *Sobir trepando com* |  |
| `c2-l012` | &emsp;*as mãos. Vt,* Yumino fazuuo yuano faza- |  |
| `c2-l013` | &emsp;mani negi tatete caqinobotta. Feiqe. |  |
| `c2-l014` | &emsp;*Lib. 1. Metendo bem a ponta do arco an-* |  |
| `c2-l015` | &emsp;*tre os penedos, ou rocha trepou.* |  |
| `c2-l016` | Caqinǒqi. *Figueira de Iapão.* |  |
| `c2-l017` | Caqinoxe, ſuru, eta. *Eſcreuer algũa couſa* |  |
| `c2-l018` | &emsp;*em papel, liuro, &c.* |  |
| `c2-l019` | Caqinoxe, ſuru, eta. *Por algũa couſa em ci* |  |
| `c2-l020` | &emsp;*ma doutra. Vt,* Xigaiuo fizano vyeni ca |  |
| `c2-l021` | &emsp;qinoxete. *Pondo o corpo morto ſobre os* |  |
| `c2-l022` | &emsp;*joelhos.* |  |
| `c2-l023` | Caqiǒ. Yoi ſacai. T*ermo, ou confins. Vt,* |  |
| `c2-l024` | &emsp;Caqiǒ ſarigataxi. *He difficultoſo apartarſe* |  |
| `c2-l025` | &emsp;*de ſeus confins, ou termo.* |  |
| `c2-l026` | Caqiocu. Vtano qiocu. *Armonia, ou ſua-* |  |
| `c2-l027` | &emsp;*uidade do canto.* |  |
| `c2-l028` | Caqiqexi, ſu, eita. *Deſaparecer. Vt,* Ca |  |
| `c2-l029` | &emsp;qiqeſu yǒni vxeta. *Deſapareceo de todo.* |  |
| `c2-l030` | Caqiqiri, ru, itta. *Certar com o corte, ou* |  |
| `c2-l031` | &emsp;*ponta pera ſi. ¶* Cubiuo caqiqiru. *Cor* |  |
| `c2-l032` | &emsp;*tar a cabeça a alguem aferrandolhe com a mão* |  |
| `c2-l033` | &emsp;*eſquerda dos cabellos, & cortãdo com o corte* |  |
| `c2-l034` | &emsp;*pera ſi.* |  |
| `c2-l035` | Caqiuo. *P*. i. Caqi. *Cerca, ou ſebe.* |  |
| `c2-l036` | Caqiſagaxi, ſu, aita. *Eſgrauatar, ou reuol* |  |
| `c2-l037` | &emsp;*uer, & buſcar algũa couſa.* |  |
| `c2-l038` | Caqiſaqi, u, aita. *Deſpedaçar com as vnhas.* |  |
| `c2-l039` | Caqiſaxi, ſu, aita. *Deixar de eſcreuer, ou de* |  |
| `c2-l040` | &emsp;*continuar com o eſcreuer.* |  |
| `c2-l041` | Caqiſoye, uru, eta. *Eſcreuendo acrecẽtar.* |  |
| `c2-l042` | Caqiſuſami, u, ôda. *Eſcreuer por deſenfada* |  |
| `c2-l043` | &emsp;*mento, ou recreação, não de propoſito.* |  |
| `c2-l044` | Caqiſute, tçuru, eta. *Deixar de eſcreuer.* |  |
| `c2-l045` | Caqiſuye, uru, eta. *Por no chão o que hum* |  |
| `c2-l046` | &emsp;*leua nas mãos, ou aos hombros. Vt,* Coxi- |  |
| `c2-l047` | &emsp;uo caqi ſuyeta. *Pazerão às andas no chão.* |  |

## Printed signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `sig-l001` | K 3 |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | Caqita |
