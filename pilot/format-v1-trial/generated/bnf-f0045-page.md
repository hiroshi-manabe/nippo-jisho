# Page view: bnf-f0045

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f45.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **A ANTES DO X.** |  |

## Column 1, before internal section

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1a-l001` | Axiricogui. *Melius.* Axicogui. *O an-* |  |
| `c1a-l002` | &emsp;*dar ao pè çepello. Vt,* Axiricoguiuo ſuru. |  |
| `c1a-l003` | &emsp;*Idem.* |  |
| `c1a-l004` | Axi ſuzume, l, Yoxiuara ſuzume. *Hum* |  |
| `c1a-l005` | &emsp;*paſſaro mayor que pardal de cor cinzenta.* |  |
| `c1a-l006` | Axita. i. Aſa. *Polamanhãa.* |  |
| `c1a-l007` | Axitacagumo. *Hũa eſpecie de aranhas de* |  |
| `c1a-l008` | &emsp;*pees muito compridos.* |  |
| `c1a-l009` | Axitatô. *Aduer. Polamanhãa cedo.* |  |
| `c1a-l010` | Axitazzu. i. tçuru. *Grow.* |  |
| `c1a-l011` | Axitçuqe. *Bandeja alta com pès como meza.* |  |
| `c1a-l012` | Axiuara, l, yoxiuara. *Lugar de muitos caniços.* |  |
| `c1a-l013` | Axiuo. *Fitas, ou cordão com que de pendurão* |  |
| `c1a-l014` | &emsp;*o montante. ¶ Item, Peozes de açor, ou* |  |
| `c1a-l015` | &emsp;*gavião.* |  |
| `c1a-l016` | Axiuonagani. *Aduerb. Modo de cingir o* |  |
| `c1a-l017` | &emsp;*montante ficando as pontas do cingidouro, ou* |  |
| `c1a-l018` | &emsp;*fitas que o ſoſtentão muito compridas.* |  |
| `c1a-l019` | Axiuoto. *Som dos pès.* |  |
| `c1a-l020` | Axixiro. *Andaimos.* |  |
| `c1a-l021` | Axizuri. *O roçar dos pees hum com outro.* |  |
| `c1a-l022` | &emsp;*¶* Axizuri, tezuri ſuru. *Rogar com* |  |
| `c1a-l023` | &emsp;*efficacia, ou moſtrar grande ſentimento.* |  |
| `c1a-l024` | Axiyri. *Atoleiro. ¶ Item, Embarcação* |  |
| `c1a-l025` | &emsp;*muito carregada, & metida no fundo. ¶* Co- |  |
| `c1a-l026` | &emsp;no funeua axiyrigia. *Eſta embarcação vem* |  |
| `c1a-l027` | &emsp;*muito metida no fundo.* |  |

## Column 1 internal heading

| Physical line | Main position | Far right |
| --- | --- | --- |
| `ih1-l001` | **A ANTES DO Y.** |  |

## Column 1, after internal section

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1b-l001` | AYA. *Laya de peça muito branda & macia* |  |
| `c1b-l002` | &emsp;*de Iapão. ¶ Item, Lauores, como folha-* |  |
| `c1b-l003` | &emsp;*gens, ou fulas que tem as peças de ſeda, &c.* |  |
| `c1b-l004` | &emsp;*¶* Ayatoru mizzuno vumote. *Mar, ou* |  |
| `c1b-l005` | &emsp;*rio de ondas creſpas como adamascadas. ¶ Itẽ,* |  |
| `c1b-l006` | &emsp;Aya. *Veas, ou neruos duros que tem o pao,* |  |
| `c1b-l007` | &emsp;*ou bambus dentro de ſi. ¶ Permet.* Ayano |  |
| `c1b-l008` | &emsp;qirenu fitogia. *He homem que não conclue* |  |
| `c1b-l009` | &emsp;*nẽ acaba de aueriguar algũ negocio.* |  |
| `c1b-l010` | Ayabumi, u, ûda. *Soſpeitar, ou duuidar.* |  |
| `c1b-l011` | &emsp;*¶* Fitono cocorouo ayabumu. *Duuidar* |  |
| `c1b-l012` | &emsp;*do coração, ou animo dalguem.* |  |
| `c1b-l013` | Ayacari. *Paruo.* |  |
| `c1b-l014` | Ayacari, u, atta. *Imitar, ou ſer ſemelhante* |  |
| `c1b-l015` | &emsp;*nos coſtumes. ¶* Fitoni ayacaru. *Parecerſe cõ* |  |
| `c1b-l016` | &emsp;*alguem nas obras, ou imitalo.* |  |
| `c1b-l017` | Ayacaxi. *Paruo.* |  |
| `c1b-l018` | Ayadori, ru, otta. *Tingir, ou pintar de di-* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **A ANTES DO Y.** |  |

## Printed page number

| Physical line | Main position | Far right |
| --- | --- | --- |
| `pn-l001` | 17. |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | &emsp;*uerſas cores, & fulas. ¶ Item, Eſcreuer com* |  |
| `c2-l002` | &emsp;*bom eſtillo, & ornato de palauras. ¶ Vt,* |  |
| `c2-l003` | &emsp;Bunxono ayadoru. *Idem.* |  |
| `c2-l004` | Ayamachi. *Deſaſtre. ¶ Item, Falta. Vt,* |  |
| `c2-l005` | &emsp;Cotobano ayamachi. *Erro nas palauras.* |  |
| `c2-l006` | Ayamachi, u, atta. *Acontecer algũ deſaſtre* |  |
| `c2-l007` | &emsp;*a alguem. ¶* Fitouo ayamatçu. *Ma-* |  |
| `c2-l008` | &emsp;*tar alguẽ. ¶ Item, Errar, ou faltar. ¶* Vo- |  |
| `c2-l009` | &emsp;moi ayamatçu. *Cuidar hũa couſa por* |  |
| `c2-l010` | &emsp;*outra.* |  |
| `c2-l011` | Ayamari. *Falta, erro, culpa. ¶* Ayama- |  |
| `c2-l012` | &emsp;riuo aratamuru, l, nauoſu. *Emendar as* |  |
| `c2-l013` | &emsp;*faltas. ¶* Ayamariuo cõ. *Pedir per-* |  |
| `c2-l014` | &emsp;*dão da culpa, ou erro.* |  |
| `c2-l015` | Ayamari, u, atta. *Errar, cair em falta, ou* |  |
| `c2-l016` | &emsp;*culpa.* |  |
| `c2-l017` | Ayame. *Eſpadana.* |  |
| `c2-l018` | Ayame. i. Yoxi axi. *Bem, ou mal dalgũa* |  |
| `c2-l019` | &emsp;*couſa. ¶ Vt,* Monono ayameuo xirazu. |  |
| `c2-l020` | &emsp;*Não ſaber dalguem, ou dalguã couſa ſe he boa* |  |
| `c2-l021` | &emsp;*ou maa.* |  |
| `c2-l022` | Ayana. i. Yeqi naxi. *Couſa vaã, & ſem* |  |
| `c2-l023` | &emsp;*proueito. P.* |  |
| `c2-l024` | Ayanaqu. *Aduerb. Sem proueito, ou em* |  |
| `c2-l025` | &emsp;*vão. P.* |  |
| `c2-l026` | Ayanaxi. *P. Couſa inutil.* |  |
| `c2-l027` | Ayanaxi, ſu. i. Ayauo naſu, l, monuo a- |  |
| `c2-l028` | &emsp;raſuru. *Pintar, ou ornar com palauras de* |  |
| `c2-l029` | &emsp;*maneira que hum folgue de ouuir. ¶* Co- |  |
| `c2-l030` | &emsp;ye ayauo naſu. *Fazer paſſos de garganta,* |  |
| `c2-l031` | &emsp;*ou, gargantear. S.* |  |
| `c2-l032` | Ayanicu, P. *Adu. Ao reuez do que homẽ* |  |
| `c2-l033` | &emsp;*deſeja, ou eſpera.* |  |
| `c2-l034` | Ayatçuri. *Inuenção, ou artificio.* |  |
| `c2-l035` | Ayatçuri, u, utta. *Inuentar, ou fazer cou-* |  |
| `c2-l036` | &emsp;*ſas de artificio, & delicadeza. ¶* Fitouo |  |
| `c2-l037` | &emsp;ayatçuru. *Enganar alguẽ contemporizando* |  |
| `c2-l038` | &emsp;*com elle, &c.* |  |
| `c2-l039` | Ayatçuri mono. *Couſa de artificio, ou enge-* |  |
| `c2-l040` | &emsp;*nhoſamente feita.* |  |
| `c2-l041` | Ayauy. *Couſa perigoſa.* |  |
| `c2-l042` | &emsp;Ayau. |  |
| `c2-l043` | &emsp;Ayauſa. |  |
| `c2-l044` | Ayaxi, ſu. *Derramar. ¶* Chiuo ayaſu. |  |
| `c2-l045` | &emsp;*Derramar ſangue. ¶ Item, No Ximo* |  |
| `c2-l046` | &emsp;*derrubar, ou fazer cair como fruita, &c.* |  |
| `c2-l047` | Ayaxij. *Couſa duuidoſa, ou eſtar duuidoſo.* |  |

## Signature

| Physical line | Main position | Far right |
| --- | --- | --- |
| `sig-l001` | E |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | Aya |
