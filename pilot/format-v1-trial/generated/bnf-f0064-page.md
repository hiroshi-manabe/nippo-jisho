# Page view: bnf-f0064

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f64.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **A ANTES DO V.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Bumecaxi, ſu, aita. *Fazer ſoido, & tra-* |  |
| `c1-l002` | &emsp;*quinada amanera do ſom de moſquitos, biſou-* |  |
| `c1-l003` | &emsp;*ros, &c. ¶ Item, Fazer ſoar algũ ins-* |  |
| `c1-l004` | &emsp;*trumento que ſe tange com a boca como buzi-* |  |
| `c1-l005` | &emsp;*na, &c.* |  |
| `c1-l006` | Bumei. *Boa dita, & ventura nas couſas da-* |  |
| `c1-l007` | &emsp;*guerra. ¶* Bumeini canǒta fito. *Ho-* |  |
| `c1-l008` | &emsp;*mem venturoſo, ou bem afortunado naguerra.* |  |
| `c1-l009` | Bumeino yubi. i. Cuſuxiyubi. *Dedo do* |  |
| `c1-l010` | &emsp;*anel, ou das mezinhas.* |  |
| `c1-l011` | Bumeqi, u, eita. *Zunirem os moſquitos, bi-* |  |
| `c1-l012` | &emsp;*zouros, &c.* |  |
| `c1-l013` | Bumo. Chichi faua. *Pay, & mãy.* |  |
| `c1-l014` | Bun. *Eſtillo de eſcreuer cartas. ¶ Item, Le* |  |
| `c1-l015` | &emsp;*tras, & ſciencia. ¶* Bunpuga nidǒ. *Le* |  |
| `c1-l016` | &emsp;*tras, & milicia, que ſão duas artes.* |  |
| `c1-l017` | Bun. vaquru, l, vacatçu. *Diuiſão, parte,* |  |
| `c1-l018` | &emsp;*quinhão, &c. ¶ Item, Ser outro, ou dif-* |  |
| `c1-l019` | &emsp;*ferente. ¶* Soreua bunde gozaru. *Ißo* |  |
| `c1-l020` | &emsp;*he differente, ou outro por ſi. ¶ Item, Poſ* |  |
| `c1-l021` | &emsp;*ſe, ou forças. Vt,* Bunni, l, Bun- |  |
| `c1-l022` | &emsp;zaini ſuguita furumai. *Conuite que* |  |
| `c1-l023` | &emsp;*excede a poſſe de quem o dà.* |  |
| `c1-l024` | Bunni. *Adu. Por ſi, ou em particular, ou* |  |
| `c1-l025` | &emsp;*differentemente.* |  |
| `c1-l026` | Bunbun. *Cada couſa por ſi, ou cada hum em* |  |
| `c1-l027` | &emsp;*particular, & a parte.* |  |
| `c1-l028` | Bunbunni. *Adu. Por ſi, ou ſeparadamente,* |  |
| `c1-l029` | &emsp;*& em particular.* |  |
| `c1-l030` | Bunchin. Fumino vomoxi. *Hum pezo que* |  |
| `c1-l031` | &emsp;*ſe poem ſobre a carta, ou ſobre o liuro pera que* |  |
| `c1-l032` | &emsp;*o vento o não leue, ou moua.* |  |
| `c1-l033` | Buncǒ. Fumino faco. *Hũa caixinha feita* |  |
| `c1-l034` | &emsp;*de pasta, ou de outra couſa pera meter cartas,* |  |
| `c1-l035` | &emsp;*& papeis.* |  |
| `c1-l036` | Bunco. Fumino cura. i. Cura. *Logea, ou ſòtão* |  |
| `c1-l037` | &emsp;*pera meter fato, liuros, &c.* |  |
| `c1-l038` | Buncocu. i. Riǒbunno cuni. *Proprios reinos,* |  |
| `c1-l039` | &emsp;*ou que pertencem a ſua jurisdição, & coroa.* |  |
| `c1-l040` | Bundai. *Hũa maneira de banquinho ſobre que* |  |
| `c1-l041` | &emsp;*poem papeis, liuros, &c.* |  |
| `c1-l042` | Bunda. *Boceta em que metem papeis. Pala-* |  |
| `c1-l043` | &emsp;*ura de molheres, & que não he propria.* |  |
| `c1-l044` | Bundan. Vaquru qire. *Partes da oração, ou* |  |
| `c1-l045` | &emsp;*preegação como exordio, narração, &c.* |  |
| `c1-l046` | Bundan. Bunuo danzuru. *Explicar texto, ou* |  |
| `c1-l047` | &emsp;*lugar eſcuro dalgum liuro.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **B ANTES DO V.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Bunen. i. nenuo irenu coto. *Pouco cuida-* |  |
| `c2-l002` | &emsp;*do, & induſtria. ¶ Item, Inconſideração.* |  |
| `c2-l003` | Bungacu. Fumi manabu. *Eſtudo, & ſcien* |  |
| `c2-l004` | &emsp;*cia de liuros, & bom estillo de cartas, &c.* |  |
| `c2-l005` | Bunguei. Fumino guei. *Arte de ſaber eſcre* |  |
| `c2-l006` | &emsp;*uer, & notar bem cartas.* |  |
| `c2-l007` | Bunguen. Cagiriuo vaquru monono fodo |  |
| `c2-l008` | &emsp;rai. *Poßibilidade, ou quantidade das cou-* |  |
| `c2-l009` | &emsp;*ſas, ou fato que hum tem. ¶ Item,* Bun |  |
| `c2-l010` | &emsp;guen. *Riqueza.* |  |
| `c2-l011` | Bunguiri. *Hum canudo em que cabe hũa car-* |  |
| `c2-l012` | &emsp;*ga de poluora. ¶ Item, Certa parte, ou* |  |
| `c2-l013` | &emsp;*quinhão de algũa couſa. ¶* Bunguiriuo ſa- |  |
| `c2-l014` | &emsp;damuru. *Diuidir, ou repartir quinhoẽs.* |  |
| `c2-l015` | Bunhô. Yutaca. *Abundancia, & abaſtança.* |  |
| `c2-l016` | Bunhôna. *Abundante, proſpero, & abastado.* |  |
| `c2-l017` | &emsp;*¶* Bunhôna cuni, toxi, &c. *Reino a-* |  |
| `c2-l018` | &emsp;*baſtado, anno proſpero, &c.* |  |
| `c2-l019` | Bunhôni. *Adu. Abundante, & proſpe-* |  |
| `c2-l020` | &emsp;*ramente.* |  |
| `c2-l021` | Bunja. Bun aru fito. *Elegante, & facun-* |  |
| `c2-l022` | &emsp;*do no eſcreuer cartas, &c. ¶ Itẽ, Permet.* |  |
| `c2-l023` | &emsp;*O que com boa prudencia, & ardil gouerna, ſem* |  |
| `c2-l024` | &emsp;*ſe aiudar muito das armas.* |  |
| `c2-l025` | Bunji. *Eſcriuão, ou ſecretario del Rei.* |  |
| `c2-l026` | Bunji, zuru, ita. *Multiplicarſe em numero,* |  |
| `c2-l027` | &emsp;*ou quantidade.* |  |
| `c2-l028` | Bunjo. *Liuros em que ſe trata do bom estillo* |  |
| `c2-l029` | &emsp;*de eſcreuer cartas, &c.* |  |
| `c2-l030` | Bunin. Fito naxi. *Pouca gente.* |  |
| `c2-l031` | Buninju. Fito cazu naxi. *Idem.* |  |
| `c2-l032` | Bunjǔ. *Nome de hum certo liuro.* |  |
| `c2-l033` | Bunmauaxi. *Compaço.* |  |
| `c2-l034` | Bunmǒ. Ca, abu. *Moſquito, & abeſpa.* |  |
| `c2-l035` | Bunpit. Fumi, fude. *Carta, & pena.* |  |
| `c2-l036` | &emsp;*¶* Bunpitni taxxita fito. *Home deſtro* |  |
| `c2-l037` | &emsp;*em eſcreuer cartas, aßi por via do estillo como* |  |
| `c2-l038` | &emsp;*de letra.* |  |
| `c2-l039` | Bunpǒ. Fumino fatto. *Regras, ou precei-* |  |
| `c2-l040` | &emsp;*tos pera ſaber bem eſcreuer cartas.* |  |
| `c2-l041` | Bunpu. i. Buntǒ, butǒ. *Letras, & mili-* |  |
| `c2-l042` | &emsp;*cia, ou ſciencia, & arte militar. ¶* Bun |  |
| `c2-l043` | &emsp;pu nidǒno fito. *Homem exercitado nas le-* |  |
| `c2-l044` | &emsp;*tras, & na milicia.* |  |
| `c2-l045` | Bunpuna. *Exercetado nas letras, & na milicia.* |  |
| `c2-l046` | Bunrai. Ca icazzuchi. i. Cano coye. *Eſ-* |  |
| `c2-l047` | &emsp;*trondo de moſquitos que fazem quando ſe ajun* |  |
| `c2-l048` | &emsp;*tão muitos.* |  |

## Catchword

| Physical line | Main position | Far right |
| --- | --- | --- |
| `catch-l001` |  | Bun |
