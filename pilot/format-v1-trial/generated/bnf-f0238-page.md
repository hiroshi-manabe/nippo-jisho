# Page view: bnf-f0238

Source: https://gallica.bnf.fr/ark:/12148/bpt6k852354j/f238.item

Scope: `full_dictionary_text_and_furniture`

## Column 1 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h1-l001` | **G ANTES DO I.** |  |

## Column 1

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c1-l001` | Guianet. *Sezoens de febres ſẽ frios.* |  |
| `c1-l002` | Guibaſǒ. *Erua aſsi chamada.* |  |
| `c1-l003` | Guibôxi. *Hũa erua, ou fula que nace della.* |  |
| `c1-l004` | Guibôxi. *Maçanetas de pao, ou latão que es-* |  |
| `c1-l005` | &emsp;*tão por remate dos balauſtes, daſpontes, &c.* |  |
| `c1-l006` | Guichǒ. *Cajado de jugar a chòca ao modo de* |  |
| `c1-l007` | &emsp;*Iapão. ¶* Guichǒde tamauo vtçu. *Dar* |  |
| `c1-l008` | &emsp;*com o cajado na choca, ou jugar a choca.* |  |
| `c1-l009` | Guidarin. i, Chauſu. *Mo de moer chà.* |  |
| `c1-l010` | Guidatte. *Modo deſe agaſtar, ou enfadar* |  |
| `c1-l011` | &emsp;*em couſas que não ha pera que ſe agastar. Vt,* |  |
| `c1-l012` | &emsp;Guidatte monouoyǔ fitogia. *Homem* |  |
| `c1-l013` | &emsp;*que ſe a gasta, ou perturba por qualquer couſa,* |  |
| `c1-l014` | &emsp;*ou toma brio deſneceßario, &c.* |  |
| `c1-l015` | Guifei. *Soldados, ou gente de guerra. ¶* Gui- |  |
| `c1-l016` | &emsp;feiuo vocoſu, l, moyouoſu. *Ajuntar* |  |
| `c1-l017` | &emsp;*gente de guerra. ¶* Guifeiuo aguru. *Idẽ.* |  |
| `c1-l018` | Guigacu. *Bailo de molheres que tem por of-* |  |
| `c1-l019` | &emsp;*ficio bailar, & recrear a outros.* |  |
| `c1-l020` | Guigio. *Molher que tem eſte officio de bai-* |  |
| `c1-l021` | &emsp;*lar, & cantar.* |  |
| `c1-l022` | Guigiǒ. *Determinação, ou aſſento, que ſe faz* |  |
| `c1-l023` | &emsp;*ſobre algũa couſa. Vt,* Guigiǒ ſuru, l, |  |
| `c1-l024` | &emsp;xita. *¶* Guigiǒ machimachi nari. |  |
| `c1-l025` | &emsp;*Auia varias determinarçoens, ou pareceres.* |  |
| `c1-l026` | Guigui. *Vt,* Guiguiſuru. *Ficar conuencido,* |  |
| `c1-l027` | &emsp;*ſem poder reſponder.* |  |
| `c1-l028` | Guigui. *Couſa grande, & magnifica. Vt,* |  |
| `c1-l029` | &emsp;Guigui tǒtǒni gozaru. *He couſa grande,* |  |
| `c1-l030` | &emsp;*& excellente, & de fausto, como onde ha mu-* |  |
| `c1-l031` | &emsp;*ta gente bem ornada por ordem, &c.* |  |
| `c1-l032` | Guijet. i, Nacauo chigǒ. *Quebrar amiza-* |  |
| `c1-l033` | &emsp;*de. Vt,* Pedroto Ioaono guijet degoza- |  |
| `c1-l034` | &emsp;ru. *Pedro, & Ioão estão quebrados, ou* |  |
| `c1-l035` | &emsp;*deſauindos.* |  |
| `c1-l036` | Guijù. Ari, fai. *Formiga, & moſca.* |  |
| `c1-l037` | Guimon. Vtagai tô. *Duuidar, & pregũtar.* |  |
| `c1-l038` | Guin. i, Xirocane. *Prata.* |  |
| `c1-l039` | Guin. *Hũa folhaſinha como de vidro, ou* |  |
| `c1-l040` | &emsp;*criſtal que poem nos perfumadores ſobre a cin-* |  |
| `c1-l041` | &emsp;*za quente, & ſobre eſta folha que às vezes* |  |
| `c1-l042` | &emsp;*he de prata, ou ouro, poẽ o cheiro, aguila, &c.* |  |
| `c1-l043` | Guinan. Ichǒnomi. *Certa fruita da aruore* |  |
| `c1-l044` | &emsp;*chamada Ichǒ, & comeſe aßada. ¶ Itẽ* |  |
| `c1-l045` | &emsp;*Ameſma aruore.* |  |
| `c1-l046` | Guinan. Xirocaneno cura. *Sella de caualo* |  |
| `c1-l047` | &emsp;*prateada.* |  |

## Column 2 running header

| Physical line | Main position | Far right |
| --- | --- | --- |
| `h2-l001` | **G ANTES DO I.** |  |

## Column 2

| Physical line | Main position | Far right |
| --- | --- | --- |
| `c2-l001` | Guinbacu. i, Xirocaneno facu. *Folhas de* |  |
| `c2-l002` | &emsp;*prata batida.* |  |
| `c2-l003` | Guinban. i, Xirocaneno ban. *Meſa, ou ta-* |  |
| `c2-l004` | &emsp;*boleiro prateado.* |  |
| `c2-l005` | Guinbiǒ. *Beǒbos prateados.* |  |
| `c2-l006` | Guinbon. Xirocaneno bon. *Bandeja ou* |  |
| `c2-l007` | &emsp;*bacio de prata.* |  |
| `c2-l008` | Guincacu. Xirocaneno iye. *Caſas ricas,* |  |
| `c2-l009` | &emsp;*& ornadas de prata.* |  |
| `c2-l010` | Guincǒ. i, Guinjite yuqu. *Ir repetindo cõ-* |  |
| `c2-l011` | &emsp;*ſigo os verſos com gosto.* |  |
| `c2-l012` | Guincǒca. *Peſſoa muito cortes.* |  |
| `c2-l013` | Guinga. i, Amanogaua. *Vialactea.* |  |
| `c2-l014` | Guinji, zuru, ita. *Repetir os verſos com* |  |
| `c2-l015` | &emsp;*ſigo com goſto, & conſideração. ¶ Item, So-* |  |
| `c2-l016` | &emsp;*ar.* Cajeni guinzuru. *Soar com o vento.* |  |
| `c2-l017` | Guinmi. Guinjite agiuǒ. *Gostar, ou prouar* |  |
| `c2-l018` | &emsp;*cõ gosto. ¶ Item, Com o entẽdimento, &* |  |
| `c2-l019` | &emsp;*vontade conſiderar cõ goſto as couſas. Vt,* |  |
| `c2-l020` | &emsp;Deusno voncotouo guinmi ſuru. *Goſtar* |  |
| `c2-l021` | &emsp;*das couſas de Deos meditandoas.* |  |
| `c2-l022` | Guinpai. Xirocaneno ſacazzuqi. *Copo, ou* |  |
| `c2-l023` | &emsp;*taça de prata.* |  |
| `c2-l024` | Guinpeqi. Xirocaneno cabe. *Parede pratea* |  |
| `c2-l025` | &emsp;*da. Vt,* Guinpeqini tamauo tçuranuru. |  |
| `c2-l026` | &emsp;*Ornar de pedras precioſas a parede prateada.* |  |
| `c2-l027` | Guinqiocu. *Armonia, ou canto ſuaue.* |  |
| `c2-l028` | Guinſan. Xirocaneno ſacazzuqi. *Copo, ou* |  |
| `c2-l029` | &emsp;*taça de prata.* |  |
| `c2-l030` | Guinſu. i, Xirocane. *Prata.* |  |
| `c2-l031` | Guinxei. Guinzuru coye. *Voz do que pro-* |  |
| `c2-l032` | &emsp;*ua verſos repetindoos, ou entoandoos cõſigo.* |  |
| `c2-l033` | Guinxẽ. Xirocaneno jeni. *Moeda de prata.* |  |
| `c2-l034` | Guinxi. Xirocaneno ito. *Fio de prata. Vt,* |  |
| `c2-l035` | &emsp;Guinxi qinchǒuo tçunagu. *Atar com fio* |  |
| `c2-l036` | &emsp;*de prata as cortinas tecidas com fio douro.* |  |
| `c2-l037` | Guinxin. Xirocaneno fari. *Agulha de pra-* |  |
| `c2-l038` | &emsp;*ta. Vt,* Guinxinuo fineru. *Meter a a-* |  |
| `c2-l039` | &emsp;*gulha deprata na carne torcendoa.* |  |
| `c2-l040` | Guinxô. Matçuni guinzuru. *Soar o vento* |  |
| `c2-l041` | &emsp;*nos pinheiros. S.* |  |
| `c2-l042` | Guinxocu. Xirocaneno tôdai. *Castiçal de* |  |
| `c2-l043` | &emsp;*prata.* |  |
| `c2-l044` | Guinyei. Guinji. *Compor, ou recitar os ver* |  |
| `c2-l045` | &emsp;*ſos entoandoos conſigo. Vt,* Xijcauo guin- |  |
| `c2-l046` | &emsp;yei ſu. *S.* |  |
| `c2-l047` | Guinzan. Xirocaneno yama. *Mõte de pra* |  |

## Printed catchword

The catchword anticipates ta. at the opening of f239, completing pra / ta across the page turn; it is not an additional body row.

| Physical line | Main position | Far right |
| --- | --- | --- |
| `u2b-l001` |  | *ta.* |
