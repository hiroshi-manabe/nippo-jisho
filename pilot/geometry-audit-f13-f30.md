...
## Text-to-image sanity check: f24–f30

After a subsequent focus failure was found at f24/c2-l041, a stronger second audit was introduced. Body-line crops were displayed as separated cards, each with its complete assigned transcription immediately below it. The reviewer had to be able to read approximately the assigned text from that crop without borrowing ink from an adjacent card. On f24 this pass retained the earlier taller override for c2-l023 and shifted c2-l041 downward so `Aixiri, u, itta. Vide Xiri, u.` is the crop's visual focus.

The same text–image sanity check was then extended through f30: 561 additional body-line crops on f25–f30, for 655 crops across f24–f30 in total. Every crop on f25–f30 passed with the existing coordinates, so no new override was needed. The review explicitly included f25's section break and enlarged initial, plus f29's irregular 45/48-line columns and enlarged initial. Both columns on f24–f30 now carry `text_image_sanity_checked`; f13–f23 retain `line_by_line_reverified` until they receive the same stronger check.
