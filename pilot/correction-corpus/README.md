...
The corpus is a diagnostic checklist, not an automatic substitution list. A category may direct attention to a location, but a change is made only when enlargement supports it. The f17 audit demonstrates the intended order: complete the page-specific passes first, then use past error families to search for omissions, record both changes and inspected non-changes, and return every candidate to the scan.
