...
Several plausible proposals were rejected rather than used to reach the target. The italic `h` in `conhecimento` resembles `b` but agrees with other same-page `h` forms; `Peſſoa`, `burǒnaru`, `Acudocu`, and the principal headword diacritics also remained supported. The audit rejected a proposed space in `Mao religioſo`, but later human line-by-line review overturned that adjudication. The same review corrected mark placement in `conſideração` and `ſaluação` and removed the spurious `i` from `tongiacu`; those later events are retained in [`f0017.tsv`](f0017.tsv).
