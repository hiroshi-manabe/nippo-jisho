...
The historical [Pilot Diacritic Audit](diacritic-audit.md) summarizes the initial glyph inventory, Unicode choices, representative source-image crops, and the history of a superseded blanket correction. Its former occurrence ledger is available through Git history rather than maintained as current project state.

The historical [`algũa` / `alguã` Tilde-Position Audit](tilde-position-audit.md) summarizes scan decisions for the 388 occurrences inventoried in its original 129-page scope. Issue #36's focused human interface overturned 86 earlier final-`a` classifications, and a later occurrence-by-occurrence pass corrected the remaining suspect readings. The former ledger is recoverable through Git history and is not a live normalization rule.

The historical [Adjacent-Vowel Tilde-Carrier Audit](adjacent-vowel-tilde-audit.md) records why the original bulk method was unreliable. Later human inspection exposed false confirmations at `maõs`, `dalgũa`, and many other forms, while subsequent Issues supplied occurrence-level corrections. Its raw inventory is now available only through Git history; a future audit must start from the current corpus.
...
The [dictionary-wide human review interface](human-review/README.md) navigates all 651 acquired leaves. Column review now trials vertically stacked, equal-width scan-line and transcription pairs with expandable context; full-page and furniture views retain the broader layout. The other 582 leaves are marked `unprocessed`, and 207 resumable human-review units track two columns and page furniture for every transcribed page. The revised `f14` units are human-checked; the corrected `f15` and `f16` columns await recheck, while `f17`–`f77` await human review.
