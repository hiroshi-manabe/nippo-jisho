...
The separate [`algũa` / `alguã` Tilde-Position Audit](tilde-position-audit.md) preserves scan decisions for the 388 occurrences inventoried in its original 129-page scope. Issue #36's focused human interface overturned 86 earlier final-`a` classifications. A later occurrence-by-occurrence scan audit corrected all 42 remaining final-`a` readings from `f101` onward; the ledger now records 372 tilde-over-`u` and 16 final-`a` forms without turning either into a lexical normalization rule.

The broader [Adjacent-Vowel Tilde-Carrier Inventory and Superseded Batch Audit](adjacent-vowel-tilde-audit.md) locates all 3,853 applicable occurrences on the not-yet-human-reviewed pages from `f39` onward, excluding the already audited `nhaã` family. Later human inspection exposed false bulk confirmations at `maõs`, `dalgũa`, and many other forms; Issue #36 supplied 128 occurrence-level corrections, and the targeted `f101+` `alguã`-family pass supplied another 42 scan-supported corrections. Unchecked rows therefore remain unverified inventory rather than individual scan decisions.
