...
The stricter geometry-only rereviews of `f44`–`f46` were imported on 2026-08-16 after the first returns proved insufficiently centered. All 277 body-line rectangles use the revised vertical positions while retaining the previously audited conservative horizontal coverage. The apparent extra final line reported on `f46` was rejected: lower-right `poſito` is a displaced continuation completing `depoſito` in `c2-l046`, so that line's crop extends downward to include it. No independent text readings were supplied or applied.
