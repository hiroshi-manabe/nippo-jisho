...
The stricter geometry-only rereviews of `f44`–`f46` were imported on 2026-08-16 after the first returns proved insufficiently centered. All 278 body-line rectangles use the revised vertical positions while retaining the previously audited conservative horizontal coverage. Human adjudication established that lower-right `poſito` on `f46` is a separate physical line continuing `depoſito` from `c2-l046`, rather than page furniture or a next-page catchword; it is therefore `c2-l047` with its own crop. No independent text readings were supplied or applied.
