...
The ordinary correction interface is the embedded line editor and page-level schema-3 GitHub Issue submission. Every prepared line displays a build-generated, read-only katakana reading hint. The editor separates a durable Level 1 line note from a temporary Message to AI; any nonempty message requires AI inspection, replacing the former second-opinion checkbox. The processor applies settled corrections to canonical Markdown or to a provisional candidate package as appropriate, then rebuilds and deploys the site.
