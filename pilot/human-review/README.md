...
For delegated geometry work from f31 onward, the complete reviewed f30 response exemplar is [`ai-geometry-examples/bnf-f0030.json`](ai-geometry-examples/bnf-f0030.json), with field semantics and the required independent-reading procedure documented in [`ai-geometry-examples/README.md`](ai-geometry-examples/README.md). The example preserves all per-line rectangles and source/version provenance expected from the other AI; its demonstration `observed_text` values are explicitly identified as canonical reference text rather than a blind second reading. Pending page records, beginning with [`ai-geometry-work/bnf-f0031.json`](ai-geometry-work/bnf-f0031.json), retain provisional coordinates but leave readings and judgments unset until the delegated inspection is performed.
