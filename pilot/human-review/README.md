...
The next scope, `f23`–`f32`, uses those 136 decisions as labeled controls. A page-held-out image-classifier experiment failed by predicting every held-out sort as short, so its output was rejected. The live UI instead begins with 28 conservative visual-comparison prechecks from [`st-ligature-prechecks-f23-f32.tsv`](../st-ligature-prechecks-f23-f32.tsv); they remain ordinary editable predictions, not accepted transcription. See the [prediction pilot report](../st-ligature-prediction-f23-f32.md).
...
For delegated geometry work from f31 onward, the [example guide](ai-geometry-examples/README.md) distinguishes a real completed review from a structural specimen. The completed [`f53` return](ai-geometry-work/bnf-f0053-reviewed.json) is the recommended behavioral model for independent reading, disagreement, uncertainty, and exceptional crop judgment. [`f30`](ai-geometry-examples/bnf-f0030.json) remains the current response-shape and coordinate specimen, but its demonstration `observed_text` values are canonical reference text rather than a blind second reading. Pending page records, beginning with [`ai-geometry-work/bnf-f0031.json`](ai-geometry-work/bnf-f0031.json), retain provisional coordinates but leave readings and judgments unset until delegated inspection is performed.
