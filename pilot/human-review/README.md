...
This generated interface supports asynchronous Level 1 production and human checking across the complete acquired Gallica sequence. All 651 leaves (`f1`–`f651`) are navigable from the outset. The six existing Level 1 pages show their transcription and review state; the remaining 645 pages show the verified scan with a clear `unprocessed` state.
