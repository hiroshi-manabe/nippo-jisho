...
Accordingly, unchanged rows are marked `batch_review_unverified`, not `scan_confirmed`. The remaining machine-found correction has status `machine_scan_corrected`, while 155 individually adjudicated occurrences have status `human_scan_confirmed`. The current ledger has 2,487 marks on `a`, 1,098 on `u`, 155 on `o`, 109 on `e`, and 4 on `i`; these are inventory totals, not verified distributions or spelling rules.
