...
Issue #36 subsequently used the dedicated one-candidate-at-a-time interface to adjudicate 128 further occurrences on 125 lines across 47 leaves. It moved 86 marks from `a` to `u`, 25 from `o` to `e`, 14 from `a` to `o`, one from `u` to `a`, one from `a` to `e`, and one between the two adjacent `a` sorts without changing the carrier letter. Seventy of the corrections are specifically `alguã` → `algũa`, demonstrating how strongly the earlier visual pass had regularized ambiguous marks toward its prior reading.

A targeted machine scan audit then revisited all 42 remaining `alguã`-family readings from `f101` onward, one enlarged occurrence at a time. Forty visibly carry the mark over `u` and were corrected; two visibly final-`a` forms, at `bnf-f0105:c1-l038` and `bnf-f0120:c2-l003`, were retained. These 40 rows are individually marked `machine_scan_corrected`; the result is evidence about those occurrences, not a license to normalize the family globally.

Accordingly, unchanged rows are marked `batch_review_unverified`, not `scan_confirmed`. The 41 individually inspected machine corrections have status `machine_scan_corrected`, while 155 individually adjudicated occurrences have status `human_scan_confirmed`. The current ledger has 2,447 marks on `a`, 1,138 on `u`, 155 on `o`, 109 on `e`, and 4 on `i`; these are inventory totals, not verified distributions or spelling rules.
...
5. This presentation proved insufficient for carrier adjudication. The replacement interface enlarges one candidate at a time, shows the current and alternate carrier explicitly, and records only a deliberate human swap; unchecked candidates remain unverified rather than being silently confirmed.
