...
Accordingly, unchanged rows are now marked `batch_review_unverified`, not `scan_confirmed`. The remaining machine-found correction has status `machine_scan_corrected`, while individually adjudicated f39–f43 corrections have status `human_scan_confirmed`. The current ledger has 2,587 marks on `a`, 1,013 on `u`, 166 on `o`, 83 on `e`, and 4 on `i`; these are inventory totals, not verified distributions or spelling rules.
