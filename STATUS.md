...
The next sequential page, [`f17`](pilot/production-review/bnf-f0017.md), applies the independent comparison together with mandatory Japanese, Portuguese, glyph, mark, and right-edge passes. Before handoff these checks caught modernized Portuguese (`conſideração`, `ſaluação`, `nascimento`), four unprinted division signs, and context-sensitive Japanese readings including `fuqeru`, `Acugaiuo`, and `Acuno`. A later open-ended autonomous audit found five further atomic scan-supported mismatches: `gosto` → `goſto`, a missing stop after `Idẽ`, `pensamen-` → `penſamen-`, and both the modernized `j` and omitted terminal stop in `preiudicial.` All 40 expected NINJAL headwords are represented. The page remains `scan_confirmed`, and its two columns and furniture await human review; its true remaining correction yield remains open until that comparison.

The exact correction history from f13 through f17 is now preserved in a [raw occurrence-level corpus](pilot/correction-corpus/README.md). Its 102 transitions retain intermediate and reverted readings as well as accepted corrections, allowing later pages to be checked by error family without turning prior answers into automatic substitutions. A first [post-hoc pattern audit on f17](pilot/correction-corpus/f0017-pattern-audit.md) rechecked the earlier sibilant, Japanese-morphology, historical-Portuguese, mark, spacing, right-edge, and repetition failure modes but accepted no additional change. The later five-error audit is recorded alongside that zero result: two missed long-`ſ` forms show that listing a category is not the same as inspecting all of its occurrences effectively. Recall remains open until human review.
