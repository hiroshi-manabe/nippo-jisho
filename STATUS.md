...
A generated [dictionary-wide human review interface](pilot/human-review/README.md) now covers all 651 acquired leaves. Its experimental column view stacks each scan line directly above its transcription at the same width, with expandable neighboring context; full-page and furniture views retain the broader paired layout. The other 642 leaves retain immediate scan access and an explicit `unprocessed` state. Previous/next controls, direct leaf entry, stable page-and-column links, and reloadable generated data allow transcription and human checking to proceed asynchronously. The review registry contains 27 resumable units: all three `f14` units are checked, both corrected `f15` and `f16` columns need recheck, and f17 human review is now in progress. Each physical line can copy its stable reference and current text into the project chat, which remains the intentionally simple correction interface.

The next public-review iteration is now specified in [Human Review and Correction Workflow](docs/human-review-workflow.md). It will add an all-page thumbnail grid, compact edit-on-click rows, comments that use otherwise free horizontal space, approximate client-side diff highlighting, and page-level correction proposals copied as JSON into GitHub Issues. The Issue body will not be encoded into the URL. Line crops must be visually adjusted so every target glyph is complete, permitting local overlap where necessary; the clipped `T` at `f17/c2-l042` is the current regression case. Public progress will report applied Issues and corrected lines rather than imply that a page has become definitively verified.
