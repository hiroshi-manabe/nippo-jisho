...
def discover():
    ...
    code=b''.join((ROOT/'scripts'/name).read_bytes() for name in ('generate_page_alignment.py','recognize_nippo_calamari.py','build_character_alignment_pilot.py'))
    ...
    for pid in sorted(set(ai)|human):
        ...
        fingerprint=hashlib.sha256(page.read_bytes()+json.dumps(geometry[pid],sort_keys=True).encode()+code+model).hexdigest()
